...
class APIBackend:
    ...
    # AC agents OBSERVE project sprints, ANALYZE outputs, fix SF-only issues.
    # They NEVER build/deploy/commit project code. See CLAUDE.md.

    def ac_list(self) -> list:
        """Return ac_cycles from /api/health (project, cycle, status, updated_at)."""
        health = self._get("/api/health")
        return health.get("ac_cycles", [])

    def ac_project(self, project_id: str) -> dict:
        """Full state for one AC pilot project: cycle, score, RL hint, convergence."""
        return self._get(f"/api/improvement/project/{project_id}")

    def ac_start(self, project_id: str) -> dict:
        """Launch project sprint + AC supervision in parallel."""
        return self._post(f"/api/improvement/start/{project_id}")

    def ac_stop(self, project_id: str) -> dict:
        """Stop a running AC supervision cycle."""
        return self._post(f"/api/improvement/stop/{project_id}")

    def ac_rollback(self, project_id: str) -> dict:
        """Rollback the last AC cycle (git revert + delete ac_cycles row)."""
        return self._post(f"/api/improvement/rollback/{project_id}")

    def ac_screenshot_url(self, project_id: str, cycle_num: int) -> str:
        """Return the URL for the QA screenshot of a given cycle."""
        return f"{self.base_url}/api/improvement/screenshot/{project_id}/{cycle_num}"

    def ac_screenshot_download(self, project_id: str, cycle_num: int) -> bytes | None:
        """Download the QA screenshot PNG for a given cycle. Returns bytes or None."""
        import httpx as _httpx

        headers = self._auth_headers()
        try:
            r = _httpx.get(
                f"{self.base_url}/api/improvement/screenshot/{project_id}/{cycle_num}",
                headers=headers,
                timeout=15,
            )
    ...
    def cockpit_activity(self, limit: int = 20) -> list:
        """Return recent agent activity from /api/cockpit/summary."""
        data = self._get("/api/cockpit/summary")
        return (data.get("activity") or [])[:limit]

    def session_checkpoints(self, session_id: str) -> list:
        """Return agent step checkpoints for a session (tool calls, progress)."""
        try:
            data = self._get(f"/api/sessions/{session_id}/checkpoints")
            return data if isinstance(data, list) else data.get("checkpoints", [])
        except Exception:
            return []

    def session_messages(self, session_id: str) -> list:
        """Return messages for a session (agent conversations)."""
        try:
            data = self._get(f"/api/sessions/{session_id}/messages/json")
    ...
    def agent_bench_list(self) -> Any:
        return self._get("/api/agent-bench/list")

    def agent_bench_run(self, agent_id: str, trials: int = 1) -> Any:
        return self._post(
            "/api/agent-bench/run", {"agent_id": agent_id, "trials": trials}
        )

    def agent_bench_job(self, job_id: str) -> Any:
        return self._get(f"/api/agent-bench/job/{job_id}")

    def agent_bench_show(self, agent_id: str) -> Any:
        return self._get(f"/api/agent-bench/{agent_id}")

    # ── Team Bench ──

    def team_bench_list(self) -> Any:
        return self._get("/api/team-bench/list")

    def team_bench_run(self, team_id: str) -> Any:
        return self._post("/api/team-bench/run", {"team_id": team_id})

    def team_bench_job(self, job_id: str) -> Any:
        return self._get(f"/api/team-bench/job/{job_id}")

    def team_bench_show(self, team_id: str) -> Any:
        return self._get(f"/api/team-bench/{team_id}")

    # ── Skill Eval ──

    def skill_eval_list(self) -> Any:
        return self._get("/api/skills/eval")

    def skill_eval_run(self, skill_name: str, trials: int = 3) -> Any:
        return self._post(f"/api/skills/eval/{skill_name}/run", {"trials": trials})

    def skill_eval_job(self, job_id: str) -> Any:
        return self._get(f"/api/skills/eval/job/{job_id}")

    def skill_eval_show(self, skill_name: str) -> Any:
        return self._get(f"/api/skills/eval/{skill_name}")
