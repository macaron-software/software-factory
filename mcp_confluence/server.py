...
Config (env):
  CONFLUENCE_URL    e.g. https://wiki.net.extra.laposte.fr/confluence
...
def _load_token() -> str:
    ...
    if not tok:
        # Try confluence.key first, then jira.key (same PAT at La Poste)
...
def _get_config() -> Dict[str, str]:
    return {
        "url": os.environ.get("CONFLUENCE_URL", "https://wiki.net.extra.laposte.fr/confluence").rstrip("/"),
        "token": _load_token(),
...
def tool_confluence_get_page(page_id: str = "", title: str = "", space: str = "") -> str:
    cfg = _get_config()
    ...
    data = _cf_get(f"content/{page_id}", {"expand": "body.storage,version,ancestors,children.page"})
    if "error" in data:
        return f"Erreur: {data['error']}"
    ...
    children = [c.get("title", "") for c in (data.get("children") or {}).get("page", {}).get("results", [])]
...
TOOLS = [
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {
                "page_id": {"type": "string", "description": "ID numérique de la page (ex: 1151407255)"},
                "title": {"type": "string", "description": "Titre exact de la page (alternatif à page_id)"},
                "space": {"type": "string", "description": "Clé espace (ex: VELIGO). Optionnel si CONFLUENCE_SPACE set."},
            },
        },
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "space": {"type": "string", "description": "Clé espace pour filtrer (ex: VELIGO)"},
    ...
    },
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {
                "space": {"type": "string", "description": "Clé espace. Optionnel si CONFLUENCE_SPACE set."},
...
class MCPConfluenceServer:
    ...
    def _dispatch(self, name: str, args: Dict) -> str:
        if name == "confluence_get_page":
            return tool_confluence_get_page(args.get("page_id", ""), args.get("title", ""), args.get("space", ""))
        if name == "confluence_search":
            return tool_confluence_search(args["query"], args.get("space", ""), args.get("max_results", 10))
        if name == "confluence_get_space":
            return tool_confluence_get_space(args.get("space", ""), args.get("max_results", 30))
