...
class TestProjectMemory:
    ...
    def test_upsert_updates_value(self, mem):
        mem.project_store(
            PROJECT_ID, "stack", "FastAPI v1", category="architecture", confidence=0.8
        )
        mem.project_store(
            PROJECT_ID, "stack", "FastAPI v2", category="architecture", confidence=0.9
        )
        entries = mem.project_get(PROJECT_ID, category="architecture")
    ...
    def test_agent_role_isolation(self, mem):
        ...
        dev_entries = mem.project_get(
            PROJECT_ID, category="convention", agent_role="dev"
        )
    ...
    def test_role_get_includes_generic(self, mem):
        ...
        entries = mem.project_get(
            PROJECT_ID, category="convention", agent_role="dev_frontend"
        )
        keys = [e["key"] for e in entries]
...
class TestCompactor:
    ...
    def test_compaction_prunes_stale_pattern(self, mem):
        ...
        from platform.db.adapter import is_postgresql

        # Insert an artificially old pattern entry
        conn = get_db()
        if is_postgresql():
            conn.execute(
                f"INSERT INTO memory_pattern (session_id, key, value, type, author_agent, created_at) "
                f"VALUES ('old-session', 'old-key', 'old-value', 'context', 'test-agent', "
                f"NOW() - INTERVAL '{MAX_PATTERN_AGE_DAYS + 1} days')"
            )
        else:
            conn.execute(
...
class TestPromptInjection:
    ...
    def test_executor_gets_role_memory(self, mem):
        ...
        agent = AgentDef(
            id="dev-be-001", name="Dev Backend", role="dev_backend", skills=[]
        )
        ...
        assert "inject-value-xyz" in prompt, (
            "Role-scoped memory should be injected in executor prompt"
        )
    ...
    def test_organizer_gets_full_memory(self, mem):
        ...
        agent = AgentDef(
            id="po-001", name="Product Owner", role="product_owner", skills=[]
        )
