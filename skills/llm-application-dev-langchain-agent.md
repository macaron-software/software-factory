...
eval_cases:
- id: llm-application-dev-langchain-agent-approach
  prompt: How should I approach llm application dev langchain agent for a production
    system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on llm application dev langchain agent
  tags:
  - llm
- id: llm-application-dev-langchain-agent-best-practices
  prompt: What are the key best practices and pitfalls for llm application dev langchain
    agent?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for llm application dev langchain agent
  tags:
  - llm
  - best-practices
- id: llm-application-dev-langchain-agent-antipatterns
  prompt: What are the most common mistakes to avoid with llm application dev langchain
    agent?
  should_trigger: true
  checks:
  - length_min:80
