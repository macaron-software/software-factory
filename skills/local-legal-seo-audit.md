...
metadata:
  ...
  triggers:
  - when working on local legal seo audit
eval_cases:
- id: local-legal-seo-audit-approach
  prompt: How should I approach local legal seo audit for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on local legal seo audit
  tags:
  - local
- id: local-legal-seo-audit-best-practices
  prompt: What are the key best practices and pitfalls for local legal seo audit?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for local legal seo audit
  tags:
  - local
  - best-practices
- id: local-legal-seo-audit-antipatterns
  prompt: What are the most common mistakes to avoid with local legal seo audit?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
