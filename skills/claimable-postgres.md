...
description: Provision instant temporary Postgres databases via Claimable Postgres
  ...
  and throwaway DATABASE_URL for prototyp...
metadata:
  category: development
  source: 'antigravity-awesome-skills (MIT) — source: community'
  triggers:
  - when working on claimable postgres
eval_cases:
- id: claimable-postgres-approach
  prompt: How should I approach claimable postgres for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on claimable postgres
  tags:
  - claimable
- id: claimable-postgres-best-practices
  prompt: What are the key best practices and pitfalls for claimable postgres?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for claimable postgres
  tags:
  - claimable
  - best-practices
- id: claimable-postgres-antipatterns
  prompt: What are the most common mistakes to avoid with claimable postgres?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
