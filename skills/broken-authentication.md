...
metadata:
  ...
  triggers:
  - when working on broken authentication
eval_cases:
- id: broken-authentication-approach
  prompt: How should I approach broken authentication for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on broken authentication
  tags:
  - broken
- id: broken-authentication-best-practices
  prompt: What are the key best practices and pitfalls for broken authentication?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for broken authentication
  tags:
  - broken
  - best-practices
- id: broken-authentication-antipatterns
  prompt: What are the most common mistakes to avoid with broken authentication?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
