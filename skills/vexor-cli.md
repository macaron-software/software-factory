...
description: Semantic file discovery via `vexor`. Use whenever locating where something
  ...
  is unclear. Prefer this over manual bro...
metadata:
  category: development
  source: 'antigravity-awesome-skills (MIT) — source: community'
  triggers:
  - when working on vexor cli
eval_cases:
- id: vexor-cli-approach
  prompt: How should I approach vexor cli for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on vexor cli
  tags:
  - vexor
- id: vexor-cli-best-practices
  prompt: What are the key best practices and pitfalls for vexor cli?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for vexor cli
  tags:
  - vexor
  - best-practices
- id: vexor-cli-antipatterns
  prompt: What are the most common mistakes to avoid with vexor cli?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
