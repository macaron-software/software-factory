...
metadata:
  ...
  triggers:
  - building durable distributed systems with temporal go sdk
  - '**designing distributed systems**: when building microservices that require dura'
  - '**implementing complex workflows**: using the go sdk to handle long-running proc'
eval_cases:
- id: temporal-golang-pro-approach
  prompt: How should I approach temporal golang pro for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on temporal golang pro
  tags:
  - temporal
- id: temporal-golang-pro-best-practices
  prompt: What are the key best practices and pitfalls for temporal golang pro?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for temporal golang pro
  tags:
  - temporal
  - best-practices
- id: temporal-golang-pro-antipatterns
  prompt: What are the most common mistakes to avoid with temporal golang pro?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
