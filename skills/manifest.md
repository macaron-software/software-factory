...
metadata:
  ...
  triggers:
  - setting up telemetry, configuring api keys, or troubleshooting the plugin
eval_cases:
- id: manifest-approach
  prompt: How should I approach manifest for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on manifest
  tags:
  - manifest
- id: manifest-best-practices
  prompt: What are the key best practices and pitfalls for manifest?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for manifest
  tags:
  - manifest
  - best-practices
- id: manifest-antipatterns
  prompt: What are the most common mistakes to avoid with manifest?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
