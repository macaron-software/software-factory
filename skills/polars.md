...
description: Fast in-memory DataFrame library for datasets that fit in RAM. Use when
  ...
  Apache Arrow backend. Best for 1-100GB...
metadata:
  category: development
  source: 'antigravity-awesome-skills (MIT) — source: community'
  triggers:
  - pandas is too slow but data still fits in memory
  - chain methods to build complex transformations
eval_cases:
- id: polars-approach
  prompt: How should I approach polars for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on polars
  tags:
  - polars
- id: polars-best-practices
  prompt: What are the key best practices and pitfalls for polars?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for polars
  tags:
  - polars
  - best-practices
- id: polars-antipatterns
  prompt: What are the most common mistakes to avoid with polars?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
