...
metadata:
  ...
  triggers:
  - when working on aws cost cleanup
eval_cases:
- id: aws-cost-cleanup-approach
  prompt: How should I approach aws cost cleanup for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on aws cost cleanup
  tags:
  - aws
- id: aws-cost-cleanup-best-practices
  prompt: What are the key best practices and pitfalls for aws cost cleanup?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for aws cost cleanup
  tags:
  - aws
  - best-practices
- id: aws-cost-cleanup-antipatterns
  prompt: What are the most common mistakes to avoid with aws cost cleanup?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
