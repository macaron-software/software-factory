...
metadata:
  ...
  triggers:
  - you need to fix ci failures, address review feedback, or continuously push fixes
eval_cases:
- id: iterate-pr-approach
  prompt: How should I approach iterate pr for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on iterate pr
  tags:
  - iterate
- id: iterate-pr-best-practices
  prompt: What are the key best practices and pitfalls for iterate pr?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for iterate pr
  tags:
  - iterate
  - best-practices
- id: iterate-pr-antipatterns
  prompt: What are the most common mistakes to avoid with iterate pr?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
