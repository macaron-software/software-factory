...
metadata:
  ...
  triggers:
  - when working on production code audit
eval_cases:
- id: production-code-audit-approach
  prompt: How should I approach production code audit for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on production code audit
  tags:
  - production
- id: production-code-audit-best-practices
  prompt: What are the key best practices and pitfalls for production code audit?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for production code audit
  tags:
  - production
  - best-practices
- id: production-code-audit-antipatterns
  prompt: What are the most common mistakes to avoid with production code audit?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
