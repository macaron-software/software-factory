...
eval_cases:
- id: bash-pro-approach
  prompt: How should I approach bash pro for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on bash pro
  tags:
  - bash
- id: bash-pro-best-practices
  prompt: What are the key best practices and pitfalls for bash pro?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for bash pro
  tags:
  - bash
  - best-practices
- id: bash-pro-antipatterns
  prompt: What are the most common mistakes to avoid with bash pro?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
