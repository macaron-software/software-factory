...
eval_cases:
- id: fp-ts-react-approach
  prompt: How should I approach fp ts react for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on fp ts react
  tags:
  - fp
- id: fp-ts-react-best-practices
  prompt: What are the key best practices and pitfalls for fp ts react?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for fp ts react
  tags:
  - fp
  - best-practices
- id: fp-ts-react-antipatterns
  prompt: What are the most common mistakes to avoid with fp ts react?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
