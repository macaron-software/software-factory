...
metadata:
  ...
  triggers:
  - when working on x twitter scraper
eval_cases:
- id: x-twitter-scraper-approach
  prompt: How should I approach x twitter scraper for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on x twitter scraper
  tags:
  - x
- id: x-twitter-scraper-best-practices
  prompt: What are the key best practices and pitfalls for x twitter scraper?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for x twitter scraper
  tags:
  - x
  - best-practices
- id: x-twitter-scraper-antipatterns
  prompt: What are the most common mistakes to avoid with x twitter scraper?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
