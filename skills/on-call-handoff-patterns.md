...
metadata:
  ...
  triggers:
  - transitioning on-call responsibilities, documenting shift summaries, or improvin
eval_cases:
- id: on-call-handoff-patterns-approach
  prompt: How should I approach on call handoff patterns for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on on call handoff patterns
  tags:
  - 'on'
- id: on-call-handoff-patterns-best-practices
  prompt: What are the key best practices and pitfalls for on call handoff patterns?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for on call handoff patterns
  tags:
  - 'on'
  - best-practices
- id: on-call-handoff-patterns-antipatterns
  prompt: What are the most common mistakes to avoid with on call handoff patterns?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
