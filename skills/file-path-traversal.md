...
metadata:
  ...
  triggers:
  - when working on file path traversal
eval_cases:
- id: file-path-traversal-approach
  prompt: How should I approach file path traversal for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on file path traversal
  tags:
  - file
- id: file-path-traversal-best-practices
  prompt: What are the key best practices and pitfalls for file path traversal?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for file path traversal
  tags:
  - file
  - best-practices
- id: file-path-traversal-antipatterns
  prompt: What are the most common mistakes to avoid with file path traversal?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
