...
metadata:
  ...
  triggers:
  - when working on fp async
eval_cases:
- id: fp-async-approach
  prompt: How should I approach fp async for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on fp async
  tags:
  - fp
- id: fp-async-best-practices
  prompt: What are the key best practices and pitfalls for fp async?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for fp async
  tags:
  - fp
  - best-practices
- id: fp-async-antipatterns
  prompt: What are the most common mistakes to avoid with fp async?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
