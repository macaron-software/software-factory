...
metadata:
  ...
  triggers:
  - when working on linux privilege escalation
eval_cases:
- id: linux-privilege-escalation-approach
  prompt: How should I approach linux privilege escalation for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on linux privilege escalation
  tags:
  - linux
- id: linux-privilege-escalation-best-practices
  prompt: What are the key best practices and pitfalls for linux privilege escalation?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for linux privilege escalation
  tags:
  - linux
  - best-practices
- id: linux-privilege-escalation-antipatterns
  prompt: What are the most common mistakes to avoid with linux privilege escalation?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
