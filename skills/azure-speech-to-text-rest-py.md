...
metadata:
  ...
  triggers:
  - when working on azure speech to text rest py
eval_cases:
- id: azure-speech-to-text-rest-py-approach
  prompt: How should I approach azure speech to text rest py for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on azure speech to text rest py
  tags:
  - azure
- id: azure-speech-to-text-rest-py-best-practices
  prompt: What are the key best practices and pitfalls for azure speech to text rest
    py?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for azure speech to text rest py
  tags:
  - azure
  - best-practices
- id: azure-speech-to-text-rest-py-antipatterns
  prompt: What are the most common mistakes to avoid with azure speech to text rest
    py?
  should_trigger: true
  checks:
  - length_min:80
