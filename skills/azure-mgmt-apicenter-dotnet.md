...
metadata:
  ...
  triggers:
  - when working on azure mgmt apicenter dotnet
eval_cases:
- id: azure-mgmt-apicenter-dotnet-approach
  prompt: How should I approach azure mgmt apicenter dotnet for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on azure mgmt apicenter dotnet
  tags:
  - azure
- id: azure-mgmt-apicenter-dotnet-best-practices
  prompt: What are the key best practices and pitfalls for azure mgmt apicenter dotnet?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for azure mgmt apicenter dotnet
  tags:
  - azure
  - best-practices
- id: azure-mgmt-apicenter-dotnet-antipatterns
  prompt: What are the most common mistakes to avoid with azure mgmt apicenter dotnet?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
