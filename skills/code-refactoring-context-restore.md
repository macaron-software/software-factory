...
metadata:
  ...
  triggers:
  - working with code refactoring context restore
  - 'working on context restoration: advanced semantic memory rehydration tasks or
    wo'
eval_cases:
- id: code-refactoring-context-restore-approach
  prompt: How should I approach code refactoring context restore for a production
    system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on code refactoring context restore
  tags:
  - code
- id: code-refactoring-context-restore-best-practices
  prompt: What are the key best practices and pitfalls for code refactoring context
    restore?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for code refactoring context restore
  tags:
  - code
  - best-practices
- id: code-refactoring-context-restore-antipatterns
  prompt: What are the most common mistakes to avoid with code refactoring context
    restore?
  should_trigger: true
  checks:
  - length_min:80
