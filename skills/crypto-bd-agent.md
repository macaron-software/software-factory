...
eval_cases:
- id: crypto-bd-agent-approach
  prompt: How should I approach crypto bd agent for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on crypto bd agent
  tags:
  - crypto
- id: crypto-bd-agent-best-practices
  prompt: What are the key best practices and pitfalls for crypto bd agent?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for crypto bd agent
  tags:
  - crypto
  - best-practices
- id: crypto-bd-agent-antipatterns
  prompt: What are the most common mistakes to avoid with crypto bd agent?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
