...
metadata:
  ...
  triggers:
  - the user is unsure which skill to use or where to start
eval_cases:
- id: skill-router-approach
  prompt: How should I approach skill router for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on skill router
  tags:
  - skill
- id: skill-router-best-practices
  prompt: What are the key best practices and pitfalls for skill router?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for skill router
  tags:
  - skill
  - best-practices
- id: skill-router-antipatterns
  prompt: What are the most common mistakes to avoid with skill router?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
