...
metadata:
  ...
  triggers:
  - the user mentions "email sequence," "drip campa
eval_cases:
- id: email-sequence-approach
  prompt: How should I approach email sequence for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on email sequence
  tags:
  - email
- id: email-sequence-best-practices
  prompt: What are the key best practices and pitfalls for email sequence?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for email sequence
  tags:
  - email
  - best-practices
- id: email-sequence-antipatterns
  prompt: What are the most common mistakes to avoid with email sequence?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
