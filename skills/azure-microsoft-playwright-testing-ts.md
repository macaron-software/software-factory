...
metadata:
  ...
  triggers:
  - scaling browser tests across cloud-hosted browsers, integrating with ci/cd pipel
eval_cases:
- id: azure-microsoft-playwright-testing-ts-approach
  prompt: How should I approach azure microsoft playwright testing ts for a production
    system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on azure microsoft playwright testing ts
  tags:
  - azure
- id: azure-microsoft-playwright-testing-ts-best-practices
  prompt: What are the key best practices and pitfalls for azure microsoft playwright
    testing ts?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for azure microsoft playwright testing ts
  tags:
  - azure
  - best-practices
- id: azure-microsoft-playwright-testing-ts-antipatterns
  prompt: What are the most common mistakes to avoid with azure microsoft playwright
    testing ts?
  should_trigger: true
  checks:
  - length_min:80
