...
metadata:
  ...
  triggers:
  - when working on tcm constitution analyzer
eval_cases:
- id: tcm-constitution-analyzer-approach
  prompt: How should I approach tcm constitution analyzer for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on tcm constitution analyzer
  tags:
  - tcm
- id: tcm-constitution-analyzer-best-practices
  prompt: What are the key best practices and pitfalls for tcm constitution analyzer?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for tcm constitution analyzer
  tags:
  - tcm
  - best-practices
- id: tcm-constitution-analyzer-antipatterns
  prompt: What are the most common mistakes to avoid with tcm constitution analyzer?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
