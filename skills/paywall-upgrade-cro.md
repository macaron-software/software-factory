...
metadata:
  ...
  triggers:
  - the user mentions "paywall," "upgrade screen," "upgrade modal,
eval_cases:
- id: paywall-upgrade-cro-approach
  prompt: How should I approach paywall upgrade cro for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on paywall upgrade cro
  tags:
  - paywall
- id: paywall-upgrade-cro-best-practices
  prompt: What are the key best practices and pitfalls for paywall upgrade cro?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for paywall upgrade cro
  tags:
  - paywall
  - best-practices
- id: paywall-upgrade-cro-antipatterns
  prompt: What are the most common mistakes to avoid with paywall upgrade cro?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
