...
description: Searches and explores Burp Suite project files (.burp) from the command
  ...
  security audit findings, dumping proxy hi...
metadata:
  category: development
  source: 'antigravity-awesome-skills (MIT) — source: community'
  triggers:
  - searching response headers or bodies with regex patterns, extracting security
    au
eval_cases:
- id: burpsuite-project-parser-approach
  prompt: How should I approach burpsuite project parser for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on burpsuite project parser
  tags:
  - burpsuite
- id: burpsuite-project-parser-best-practices
  prompt: What are the key best practices and pitfalls for burpsuite project parser?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for burpsuite project parser
  tags:
  - burpsuite
  - best-practices
- id: burpsuite-project-parser-antipatterns
  prompt: What are the most common mistakes to avoid with burpsuite project parser?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
