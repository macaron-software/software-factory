...
eval_cases:
- id: database-admin-approach
  prompt: How should I approach database admin for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on database admin
  tags:
  - database
- id: database-admin-best-practices
  prompt: What are the key best practices and pitfalls for database admin?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for database admin
  tags:
  - database
  - best-practices
- id: database-admin-antipatterns
  prompt: What are the most common mistakes to avoid with database admin?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
