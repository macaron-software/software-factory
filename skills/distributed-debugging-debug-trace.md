...
eval_cases:
- id: distributed-debugging-debug-trace-approach
  prompt: How should I approach distributed debugging debug trace for a production
    system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on distributed debugging debug trace
  tags:
  - distributed
- id: distributed-debugging-debug-trace-best-practices
  prompt: What are the key best practices and pitfalls for distributed debugging debug
    trace?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for distributed debugging debug trace
  tags:
  - distributed
  - best-practices
- id: distributed-debugging-debug-trace-antipatterns
  prompt: What are the most common mistakes to avoid with distributed debugging debug
    trace?
  should_trigger: true
  checks:
  - length_min:80
