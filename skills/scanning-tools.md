...
metadata:
  ...
  triggers:
  - when working on scanning tools
eval_cases:
- id: scanning-tools-approach
  prompt: How should I approach scanning tools for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on scanning tools
  tags:
  - scanning
- id: scanning-tools-best-practices
  prompt: What are the key best practices and pitfalls for scanning tools?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for scanning tools
  tags:
  - scanning
  - best-practices
- id: scanning-tools-antipatterns
  prompt: What are the most common mistakes to avoid with scanning tools?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
