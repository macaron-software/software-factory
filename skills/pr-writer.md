...
description: ALWAYS use this skill when creating or updating pull requests — never
  ...
  descriptions, and issue references. Trigger ...
metadata:
  category: development
  source: 'antigravity-awesome-skills (MIT) — source: community'
  triggers:
  - when working on pr writer
eval_cases:
- id: pr-writer-approach
  prompt: How should I approach pr writer for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on pr writer
  tags:
  - pr
- id: pr-writer-best-practices
  prompt: What are the key best practices and pitfalls for pr writer?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for pr writer
  tags:
  - pr
  - best-practices
- id: pr-writer-antipatterns
  prompt: What are the most common mistakes to avoid with pr writer?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
