...
description: Direct REST API access to PubMed. Advanced Boolean/MeSH queries, E-utilities
  ...
  (Bio.Entrez). Use this for direct HTT...
metadata:
...
eval_cases:
- id: pubmed-database-approach
  prompt: How should I approach pubmed database for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on pubmed database
  tags:
  - pubmed
- id: pubmed-database-best-practices
  prompt: What are the key best practices and pitfalls for pubmed database?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for pubmed database
  tags:
  - pubmed
  - best-practices
- id: pubmed-database-antipatterns
  prompt: What are the most common mistakes to avoid with pubmed database?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
