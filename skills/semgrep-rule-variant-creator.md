...
description: Creates language variants of existing Semgrep rules. Use when porting
  ...
  languages as input, produces independent rule+t...
metadata:
...
eval_cases:
- id: semgrep-rule-variant-creator-approach
  prompt: How should I approach semgrep rule variant creator for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on semgrep rule variant creator
  tags:
  - semgrep
- id: semgrep-rule-variant-creator-best-practices
  prompt: What are the key best practices and pitfalls for semgrep rule variant creator?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for semgrep rule variant creator
  tags:
  - semgrep
  - best-practices
- id: semgrep-rule-variant-creator-antipatterns
  prompt: What are the most common mistakes to avoid with semgrep rule variant creator?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
