...
metadata:
  ...
  triggers:
  - securing kubernetes clusters, implementing network isolation, or
eval_cases:
- id: k8s-security-policies-approach
  prompt: How should I approach k8s security policies for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on k8s security policies
  tags:
  - k8s
- id: k8s-security-policies-best-practices
  prompt: What are the key best practices and pitfalls for k8s security policies?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for k8s security policies
  tags:
  - k8s
  - best-practices
- id: k8s-security-policies-antipatterns
  prompt: What are the most common mistakes to avoid with k8s security policies?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
