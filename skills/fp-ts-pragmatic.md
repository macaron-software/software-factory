...
metadata:
  category: ai
  source: 'antigravity-awesome-skills (MIT) — source: https://github.com/whatiskadudoing/fp-ts-skills'
  triggers:
  - writing typescript with fp-ts library
  - when writing typescript code that handles nullable values, errors, or async oper
eval_cases:
- id: fp-ts-pragmatic-approach
  prompt: How should I approach fp ts pragmatic for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on fp ts pragmatic
  tags:
  - fp
- id: fp-ts-pragmatic-best-practices
  prompt: What are the key best practices and pitfalls for fp ts pragmatic?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for fp ts pragmatic
  tags:
  - fp
  - best-practices
- id: fp-ts-pragmatic-antipatterns
  prompt: What are the most common mistakes to avoid with fp ts pragmatic?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
