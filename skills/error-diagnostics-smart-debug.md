...
metadata:
  ...
  triggers:
  - working with error diagnostics smart debug
  - working on error diagnostics smart debug tasks or workflows
  - the task is unrelated to error diagnostics smart debug
eval_cases:
- id: error-diagnostics-smart-debug-approach
  prompt: How should I approach error diagnostics smart debug for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on error diagnostics smart debug
  tags:
  - error
- id: error-diagnostics-smart-debug-best-practices
  prompt: What are the key best practices and pitfalls for error diagnostics smart
    debug?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for error diagnostics smart debug
  tags:
  - error
  - best-practices
- id: error-diagnostics-smart-debug-antipatterns
  prompt: What are the most common mistakes to avoid with error diagnostics smart
    debug?
  should_trigger: true
  checks:
  - length_min:80
