...
metadata:
  category: ai
  source: 'antigravity-awesome-skills (MIT) — source: https://github.com/k-kolomeitsev/data-structure-protocol'
  triggers:
  - creating, modifying, or deleting code files in a dsp-tracked project (to keep
    th
eval_cases:
- id: data-structure-protocol-approach
  prompt: How should I approach data structure protocol for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on data structure protocol
  tags:
  - data
- id: data-structure-protocol-best-practices
  prompt: What are the key best practices and pitfalls for data structure protocol?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for data structure protocol
  tags:
  - data
  - best-practices
- id: data-structure-protocol-antipatterns
  prompt: What are the most common mistakes to avoid with data structure protocol?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
