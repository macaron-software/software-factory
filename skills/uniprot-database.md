...
description: Direct REST API access to UniProt. Protein searches, FASTA retrieval,
  ...
  bioservices (unified interface to 40+ servi...
metadata:
  category: development
  source: 'antigravity-awesome-skills (MIT) — source: community'
  triggers:
  - when working on uniprot database
eval_cases:
- id: uniprot-database-approach
  prompt: How should I approach uniprot database for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on uniprot database
  tags:
  - uniprot
- id: uniprot-database-best-practices
  prompt: What are the key best practices and pitfalls for uniprot database?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for uniprot database
  tags:
  - uniprot
  - best-practices
- id: uniprot-database-antipatterns
  prompt: What are the most common mistakes to avoid with uniprot database?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
