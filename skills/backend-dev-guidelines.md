...
metadata:
  ...
  triggers:
  - when working on backend dev guidelines
eval_cases:
- id: backend-dev-guidelines-approach
  prompt: How should I approach backend dev guidelines for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on backend dev guidelines
  tags:
  - backend
- id: backend-dev-guidelines-best-practices
  prompt: What are the key best practices and pitfalls for backend dev guidelines?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for backend dev guidelines
  tags:
  - backend
  - best-practices
- id: backend-dev-guidelines-antipatterns
  prompt: What are the most common mistakes to avoid with backend dev guidelines?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
