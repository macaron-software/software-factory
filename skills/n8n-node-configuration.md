...
metadata:
  ...
  triggers:
  - configuring nodes, understanding property dependencies, determining required fie
eval_cases:
- id: n8n-node-configuration-approach
  prompt: How should I approach n8n node configuration for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on n8n node configuration
  tags:
  - n8n
- id: n8n-node-configuration-best-practices
  prompt: What are the key best practices and pitfalls for n8n node configuration?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for n8n node configuration
  tags:
  - n8n
  - best-practices
- id: n8n-node-configuration-antipatterns
  prompt: What are the most common mistakes to avoid with n8n node configuration?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
