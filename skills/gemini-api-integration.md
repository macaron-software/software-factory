...
metadata:
  ...
  - use when setting up gemini api for the first time in a node.js, python, or brows
  - use when implementing multimodal inputs (text + image/audio/video)
eval_cases:
- id: gemini-api-integration-approach
  prompt: How should I approach gemini api integration for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on gemini api integration
  tags:
  - gemini
- id: gemini-api-integration-best-practices
  prompt: What are the key best practices and pitfalls for gemini api integration?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for gemini api integration
  tags:
  - gemini
  - best-practices
- id: gemini-api-integration-antipatterns
  prompt: What are the most common mistakes to avoid with gemini api integration?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
