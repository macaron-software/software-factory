...
metadata:
  ...
  triggers:
  - building event sourcing infrastructure, choosing event store technologies, or
    im
  - the task is unrelated to event store design
eval_cases:
- id: event-store-design-approach
  prompt: How should I approach event store design for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on event store design
  tags:
  - event
- id: event-store-design-best-practices
  prompt: What are the key best practices and pitfalls for event store design?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for event store design
  tags:
  - event
  - best-practices
- id: event-store-design-antipatterns
  prompt: What are the most common mistakes to avoid with event store design?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
