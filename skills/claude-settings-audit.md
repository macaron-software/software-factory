...
description: Analyze a repository to generate recommended Claude Code settings.json
  ...
  which read-only bash commands to al...
metadata:
  category: development
  source: 'antigravity-awesome-skills (MIT) — source: community'
  triggers:
  - setting up a new project, auditing existing settings, or determining which read-
eval_cases:
- id: claude-settings-audit-approach
  prompt: How should I approach claude settings audit for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on claude settings audit
  tags:
  - claude
- id: claude-settings-audit-best-practices
  prompt: What are the key best practices and pitfalls for claude settings audit?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for claude settings audit
  tags:
  - claude
  - best-practices
- id: claude-settings-audit-antipatterns
  prompt: What are the most common mistakes to avoid with claude settings audit?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
