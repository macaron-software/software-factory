...
metadata:
  ...
  triggers:
  - creating, encrypting/decrypting, signing, or rotating keys
eval_cases:
- id: azure-keyvault-keys-ts-approach
  prompt: How should I approach azure keyvault keys ts for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on azure keyvault keys ts
  tags:
  - azure
- id: azure-keyvault-keys-ts-best-practices
  prompt: What are the key best practices and pitfalls for azure keyvault keys ts?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for azure keyvault keys ts
  tags:
  - azure
  - best-practices
- id: azure-keyvault-keys-ts-antipatterns
  prompt: What are the most common mistakes to avoid with azure keyvault keys ts?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
