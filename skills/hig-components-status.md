...
metadata:
  ...
  triggers:
  - when working on hig components status
eval_cases:
- id: hig-components-status-approach
  prompt: How should I approach hig components status for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on hig components status
  tags:
  - hig
- id: hig-components-status-best-practices
  prompt: What are the key best practices and pitfalls for hig components status?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for hig components status
  tags:
  - hig
  - best-practices
- id: hig-components-status-antipatterns
  prompt: What are the most common mistakes to avoid with hig components status?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
