...
metadata:
  ...
  triggers:
  - when working on microsoft azure webjobs extensions authentication events dotnet
eval_cases:
- id: microsoft-azure-webjobs-extensions-authentication-events-dotnet-approach
  prompt: How should I approach microsoft azure webjobs extensions authentication
    events dotnet for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on microsoft azure webjobs extensions authentication
    events dotnet
  tags:
  - microsoft
- id: microsoft-azure-webjobs-extensions-authentication-events-dotnet-best-practices
  prompt: What are the key best practices and pitfalls for microsoft azure webjobs
    extensions authentication events dotnet?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for microsoft azure webjobs extensions authentication
    events dotnet
  tags:
  - microsoft
  - best-practices
- id: microsoft-azure-webjobs-extensions-authentication-events-dotnet-antipatterns
  prompt: What are the most common mistakes to avoid with microsoft azure webjobs
    extensions authentication events dotnet?
  should_trigger: true
  checks:
  - length_min:80
