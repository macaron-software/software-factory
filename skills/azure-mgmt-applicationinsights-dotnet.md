...
metadata:
  ...
  triggers:
  - when working on azure mgmt applicationinsights dotnet
eval_cases:
- id: azure-mgmt-applicationinsights-dotnet-approach
  prompt: How should I approach azure mgmt applicationinsights dotnet for a production
    system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on azure mgmt applicationinsights dotnet
  tags:
  - azure
- id: azure-mgmt-applicationinsights-dotnet-best-practices
  prompt: What are the key best practices and pitfalls for azure mgmt applicationinsights
    dotnet?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for azure mgmt applicationinsights dotnet
  tags:
  - azure
  - best-practices
- id: azure-mgmt-applicationinsights-dotnet-antipatterns
  prompt: What are the most common mistakes to avoid with azure mgmt applicationinsights
    dotnet?
  should_trigger: true
  checks:
  - length_min:80
