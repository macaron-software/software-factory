...
metadata:
  ...
  triggers:
  - configuring authentication with defaultazurecredential, managed identity, servic
eval_cases:
- id: azure-identity-ts-approach
  prompt: How should I approach azure identity ts for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on azure identity ts
  tags:
  - azure
- id: azure-identity-ts-best-practices
  prompt: What are the key best practices and pitfalls for azure identity ts?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for azure identity ts
  tags:
  - azure
  - best-practices
- id: azure-identity-ts-antipatterns
  prompt: What are the most common mistakes to avoid with azure identity ts?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
