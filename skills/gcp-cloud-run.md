...
metadata:
  ...
  source: 'antigravity-awesome-skills (MIT) — source: vibeship-spawner-skills (Apache
    2.0)'
  triggers:
  - when working on gcp cloud run
eval_cases:
- id: gcp-cloud-run-approach
  prompt: How should I approach gcp cloud run for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on gcp cloud run
  tags:
  - gcp
- id: gcp-cloud-run-best-practices
  prompt: What are the key best practices and pitfalls for gcp cloud run?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for gcp cloud run
  tags:
  - gcp
  - best-practices
- id: gcp-cloud-run-antipatterns
  prompt: What are the most common mistakes to avoid with gcp cloud run?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
