...
metadata:
  ...
  source: 'antigravity-awesome-skills (MIT) — source: https://github.com/wrsmith108/linear-claude-skill'
  triggers:
  - when working on linear claude skill
eval_cases:
- id: linear-claude-skill-approach
  prompt: How should I approach linear claude skill for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on linear claude skill
  tags:
  - linear
- id: linear-claude-skill-best-practices
  prompt: What are the key best practices and pitfalls for linear claude skill?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for linear claude skill
  tags:
  - linear
  - best-practices
- id: linear-claude-skill-antipatterns
  prompt: What are the most common mistakes to avoid with linear claude skill?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
