...
metadata:
  ...
  triggers:
  - the user wants to create a browsa
  - user asks to "build a site" or "package as vitepress"
  - user runs the `/deep-wiki:build` command
eval_cases:
- id: wiki-vitepress-approach
  prompt: How should I approach wiki vitepress for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on wiki vitepress
  tags:
  - wiki
- id: wiki-vitepress-best-practices
  prompt: What are the key best practices and pitfalls for wiki vitepress?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for wiki vitepress
  tags:
  - wiki
  - best-practices
- id: wiki-vitepress-antipatterns
  prompt: What are the most common mistakes to avoid with wiki vitepress?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
