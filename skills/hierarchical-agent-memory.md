...
metadata:
  category: ai
  source: 'antigravity-awesome-skills (MIT) — source: https://github.com/kromahlusenii-ops/ham'
  triggers:
  - when working on hierarchical agent memory
eval_cases:
- id: hierarchical-agent-memory-approach
  prompt: How should I approach hierarchical agent memory for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on hierarchical agent memory
  tags:
  - hierarchical
- id: hierarchical-agent-memory-best-practices
  prompt: What are the key best practices and pitfalls for hierarchical agent memory?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for hierarchical agent memory
  tags:
  - hierarchical
  - best-practices
- id: hierarchical-agent-memory-antipatterns
  prompt: What are the most common mistakes to avoid with hierarchical agent memory?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
