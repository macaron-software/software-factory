...
metadata:
  ...
  triggers:
  - when working on api fuzzing bug bounty
eval_cases:
- id: api-fuzzing-bug-bounty-approach
  prompt: How should I approach api fuzzing bug bounty for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on api fuzzing bug bounty
  tags:
  - api
- id: api-fuzzing-bug-bounty-best-practices
  prompt: What are the key best practices and pitfalls for api fuzzing bug bounty?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for api fuzzing bug bounty
  tags:
  - api
  - best-practices
- id: api-fuzzing-bug-bounty-antipatterns
  prompt: What are the most common mistakes to avoid with api fuzzing bug bounty?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
