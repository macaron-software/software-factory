...
metadata:
  ...
  triggers:
  - when working on goal analyzer
eval_cases:
- id: goal-analyzer-approach
  prompt: How should I approach goal analyzer for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on goal analyzer
  tags:
  - goal
- id: goal-analyzer-best-practices
  prompt: What are the key best practices and pitfalls for goal analyzer?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for goal analyzer
  tags:
  - goal
  - best-practices
- id: goal-analyzer-antipatterns
  prompt: What are the most common mistakes to avoid with goal analyzer?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
