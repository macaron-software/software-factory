...
description: Debugs the Buttercup CRS (Cyber Reasoning System) running on Kubernetes.
  ...
  disk saturation, DinD issues, or any serv...
metadata:
  category: ops
  source: 'antigravity-awesome-skills (MIT) — source: community'
  triggers:
  - diagnosing pod crashes, restart loops, redis failures, resource pressure, disk
    s
eval_cases:
- id: debug-buttercup-approach
  prompt: How should I approach debug buttercup for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on debug buttercup
  tags:
  - debug
- id: debug-buttercup-best-practices
  prompt: What are the key best practices and pitfalls for debug buttercup?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for debug buttercup
  tags:
  - debug
  - best-practices
- id: debug-buttercup-antipatterns
  prompt: What are the most common mistakes to avoid with debug buttercup?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
