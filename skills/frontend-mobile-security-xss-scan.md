...
eval_cases:
- id: frontend-mobile-security-xss-scan-approach
  prompt: How should I approach frontend mobile security xss scan for a production
    system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on frontend mobile security xss scan
  tags:
  - frontend
- id: frontend-mobile-security-xss-scan-best-practices
  prompt: What are the key best practices and pitfalls for frontend mobile security
    xss scan?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for frontend mobile security xss scan
  tags:
  - frontend
  - best-practices
- id: frontend-mobile-security-xss-scan-antipatterns
  prompt: What are the most common mistakes to avoid with frontend mobile security
    xss scan?
  should_trigger: true
  checks:
  - length_min:80
