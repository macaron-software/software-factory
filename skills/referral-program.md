...
metadata:
  ...
  triggers:
  - the user mentions 'referral,' 'affiliate,' 'ambassador,' 'word of
  - are you building a customer referral program, affiliate program, or both?
eval_cases:
- id: referral-program-approach
  prompt: How should I approach referral program for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on referral program
  tags:
  - referral
- id: referral-program-best-practices
  prompt: What are the key best practices and pitfalls for referral program?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for referral program
  tags:
  - referral
  - best-practices
- id: referral-program-antipatterns
  prompt: What are the most common mistakes to avoid with referral program?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
