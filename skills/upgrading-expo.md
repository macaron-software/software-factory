...
metadata:
  ...
  source: 'antigravity-awesome-skills (MIT) — source: https://github.com/expo/skills/tree/main/plugins/upgrading-expo'
  triggers:
  - when working on upgrading expo
eval_cases:
- id: upgrading-expo-approach
  prompt: How should I approach upgrading expo for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on upgrading expo
  tags:
  - upgrading
- id: upgrading-expo-best-practices
  prompt: What are the key best practices and pitfalls for upgrading expo?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for upgrading expo
  tags:
  - upgrading
  - best-practices
- id: upgrading-expo-antipatterns
  prompt: What are the most common mistakes to avoid with upgrading expo?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
