...
metadata:
  ...
  triggers:
  - mcp__claude-in-chrome__* tools fail, return "browser extension is not connected"
eval_cases:
- id: claude-in-chrome-troubleshooting-approach
  prompt: How should I approach claude in chrome troubleshooting for a production
    system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on claude in chrome troubleshooting
  tags:
  - claude
- id: claude-in-chrome-troubleshooting-best-practices
  prompt: What are the key best practices and pitfalls for claude in chrome troubleshooting?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for claude in chrome troubleshooting
  tags:
  - claude
  - best-practices
- id: claude-in-chrome-troubleshooting-antipatterns
  prompt: What are the most common mistakes to avoid with claude in chrome troubleshooting?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
