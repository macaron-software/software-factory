...
metadata:
  ...
  triggers:
  - when working on occupational health analyzer
eval_cases:
- id: occupational-health-analyzer-approach
  prompt: How should I approach occupational health analyzer for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on occupational health analyzer
  tags:
  - occupational
- id: occupational-health-analyzer-best-practices
  prompt: What are the key best practices and pitfalls for occupational health analyzer?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for occupational health analyzer
  tags:
  - occupational
  - best-practices
- id: occupational-health-analyzer-antipatterns
  prompt: What are the most common mistakes to avoid with occupational health analyzer?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
