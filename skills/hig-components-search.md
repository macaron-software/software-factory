...
metadata:
  ...
  triggers:
  - when working on hig components search
eval_cases:
- id: hig-components-search-approach
  prompt: How should I approach hig components search for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on hig components search
  tags:
  - hig
- id: hig-components-search-best-practices
  prompt: What are the key best practices and pitfalls for hig components search?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for hig components search
  tags:
  - hig
  - best-practices
- id: hig-components-search-antipatterns
  prompt: What are the most common mistakes to avoid with hig components search?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
