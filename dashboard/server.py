...
SKILL_EVALS_DIR = BASE_DIR / "data" / "skill_evals"

# Verdict thresholds (must match skill_eval_tools.py)
_PASS_RATE_GREEN = 0.80
_PASS_RATE_YELLOW = 0.50


def _load_skill_categories() -> dict[str, str]:
    """Read category from each skill's YAML frontmatter."""
    cats: dict[str, str] = {}
    ...
    for f in SKILLS_DIR.glob("*.md"):
        try:
            ...
            cats[f.stem] = meta.get("category", "other")
        except Exception:
            cats[f.stem] = "other"
    return cats


def _load_all_skill_evals() -> list[dict]:
    """Load all skill eval JSON results merged with their category."""
    if not SKILL_EVALS_DIR.exists():
        return []
    categories = _load_skill_categories()
    results = []
    for f in sorted(SKILL_EVALS_DIR.glob("*.json")):
        try:
            d = json.loads(f.read_text(encoding="utf-8"))
            name = d.get("skill_name", f.stem)
            pr = d.get("pass_rate")
            if pr is None:
                verdict = "never_run"
            elif pr >= _PASS_RATE_GREEN:
                verdict = "pass"
            elif pr >= _PASS_RATE_YELLOW:
                verdict = "warn"
            else:
                verdict = "fail"
            results.append(
                {
                    ...
                    "eval_cases_total": d.get("eval_cases_total", 0),
                    "pass_rate": pr,
                    "verdict": verdict,
                    "status": d.get("status", "ok"),
                    "ran_at": d.get("ran_at", ""),
                    "duration_s": d.get("duration_s", 0),
                    "retirement_signal": d.get("retirement_signal", False),
                    "coverage_gap": d.get("coverage_gap", []),
                    # aggregated case metrics
                    "checks_avg": (
                        round(
                            sum(
                                c.get("checks_pass_rate", 0)
                                for c in d.get("case_results", [])
                            )
                            / max(1, len(d.get("case_results", []))),
                            3,
                        )
                        if d.get("case_results")
                        else None
                    ),
                    "llm_avg": (
                        round(
                            sum(
                                c.get("llm_judge_score", 0)
                                for c in d.get("case_results", [])
                            )
                            / max(1, len(d.get("case_results", []))),
                            3,
...
async def api_skills_summary():
    """Aggregate stats for the Skills Health board."""
    skills = await asyncio.to_thread(_load_all_skill_evals)
    by_verdict = {"pass": 0, "warn": 0, "fail": 0, "never_run": 0}
    ...
    for s in skills:
        ...
        if cat not in by_category:
            by_category[cat] = {"total": 0, "pass": 0, "warn": 0, "fail": 0}
        by_category[cat]["total"] += 1
        by_category[cat][v] = by_category[cat].get(v, 0) + 1
        if s["ran_at"] and s["ran_at"] > last_run:
            last_run = s["ran_at"]

    total = len(skills)
    evaluated = total - by_verdict.get("never_run", 0)
    pass_count = by_verdict.get("pass", 0)
    # Category pass rates
    for cat, d in by_category.items():
        t = d["total"]
        d["pass_rate"] = round(d["pass"] / t, 3) if t else 0

    return {
        "total": total,
        "evaluated": evaluated,
        "never_run": by_verdict.get("never_run", 0),
        "pass": pass_count,
        "warn": by_verdict.get("warn", 0),
        "fail": by_verdict.get("fail", 0),
        "pass_pct": round(pass_count / max(1, evaluated) * 100, 1),
...
async def api_skills_list(
    ...
    sort: str = Query("pass_rate"),  # pass_rate | ran_at | name
    order: str = Query("asc"),
):
    """Paginated, filterable skill list with eval metrics."""
    skills = await asyncio.to_thread(_load_all_skill_evals)

    # Filter
    if q:
        q_low = q.lower()
        skills = [s for s in skills if q_low in s["skill_name"].lower()]
    if category:
        skills = [s for s in skills if s["category"] == category]
    if verdict:
        skills = [s for s in skills if s["verdict"] == verdict]

    # Sort
    reverse = order == "desc"
    if sort == "pass_rate":
        skills.sort(key=lambda s: (s["pass_rate"] or -1), reverse=reverse)
    elif sort == "ran_at":
        skills.sort(key=lambda s: s["ran_at"] or "", reverse=reverse)
    elif sort == "name":
        skills.sort(key=lambda s: s["skill_name"], reverse=reverse)
    elif sort == "checks":
        skills.sort(key=lambda s: (s["checks_avg"] or -1), reverse=reverse)
    elif sort == "llm":
        skills.sort(key=lambda s: (s["llm_avg"] or -1), reverse=reverse)

    total = len(skills)
    start = (page - 1) * per_page
    page_items = skills[start : start + per_page]
    return {"skills": page_items, "total": total, "page": page, "per_page": per_page}


@app.get("/api/skills/{skill_name}/detail")
async def api_skill_detail(skill_name: str):
    """Full eval detail for one skill — case_results with check_details and judge_notes."""
    eval_file = SKILL_EVALS_DIR / f"{skill_name}.json"
    if not eval_file.exists():
        return {"error": "not found"}
    d = json.loads(eval_file.read_text(encoding="utf-8"))
    categories = await asyncio.to_thread(_load_skill_categories)
    d["category"] = categories.get(skill_name, "other")
    return d


@app.get("/skills-health", response_class=HTMLResponse)
async def skills_health_page(request: Request):
    """Skills Health board — eval results, deterministic checks, LLM judge."""
    summary = await api_skills_summary()
    return templates.TemplateResponse(
        "skills-health.html",
