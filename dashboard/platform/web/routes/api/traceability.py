...
from ....traceability.store import get_why, get_session_why

router = APIRouter()


@router.get("/api/traceability")
async def get_artifact_why(artifact: str = ""):
    ...
    return {"artifact": artifact, "why_chain": [
        {"artifact_type": e.artifact_type, "lineage_chain": e.lineage_chain,
         "rationale": e.rationale, "created_at": e.created_at}
        for e in entries
    ]}


@router.get("/api/traceability/{session_id}")
async def get_session_traceability(session_id: str):
    ...
    return {"session_id": session_id, "artifacts": by_type, "total": len(entries)}
