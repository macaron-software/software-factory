...
from .service import User, verify_access_token, user_count, get_project_role
...
def _check_api_key(request: Request) -> bool:
    ...
    if not token:
        return False
    return hashlib.sha256(token.encode()).hexdigest() == hashlib.sha256(_API_KEY.encode()).hexdigest()
