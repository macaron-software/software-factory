...
def test_chaos_pause_resume(az_base_url: str, ssh_run_az: Callable) -> None:
    ...
    try:
        r = _get(f"{az_base_url.replace('52.143.158.19', '20.19.35.21')}/api/health", timeout=8)
    ...
    if not lb_ok_during_pause:
        pytest.xfail("LB did not serve during 5s SIGSTOP (health probe interval may exceed pause duration)")
