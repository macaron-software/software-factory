...
Stability tests require STABILITY_TESTS=1 env var + optional host/SSH vars:
  STABILITY_AZ_HOST      — Azure SF node-1 base URL (default: http://52.143.158.19)
  STABILITY_OVH_HOST     — OVH demo base URL (default: http://54.36.183.124)
  STABILITY_LB_HOST      — Azure LB public IP (default: http://20.19.35.21)
...
def az_base_url() -> str:
    return os.environ.get("STABILITY_AZ_HOST", "http://52.143.158.19").rstrip("/")


@pytest.fixture(scope="session")
def az_node2_url() -> str:
    return os.environ.get("STABILITY_AZ_NODE2_HOST", "http://40.89.174.75").rstrip("/")


@pytest.fixture(scope="session")
def lb_base_url() -> str:
    return os.environ.get("STABILITY_LB_HOST", "http://20.19.35.21").rstrip("/")


@pytest.fixture(scope="session")
def ovh_base_url() -> str:
    return os.environ.get("STABILITY_OVH_HOST", "https://sf.macaron-software.com").rstrip("/")
...
def ssh_run_az(ssh_key_az: str | None):
    ...
    def _run(cmd: str) -> tuple[int, str, str]:
        return _ssh_run("sfadmin@52.143.158.19", cmd, ssh_key_az)
    return _run
...
def ssh_run_az2(ssh_key_az: str | None):
    ...
    def _run(cmd: str) -> tuple[int, str, str]:
        return _ssh_run("sfadmin@40.89.174.75", cmd, ssh_key_az)
    return _run
...
def ssh_run_ovh(ssh_key_ovh: str | None):
    ...
    def _run(cmd: str) -> tuple[int, str, str]:
        return _ssh_run("debian@54.36.183.124", cmd, ssh_key_ovh)
    return _run
