...
async def agent_details_json(agent_id: str):
    ...
    avatar_url = (
        f"/static/avatars/{a.id}.jpg" if (avatars_dir / f"{a.id}.jpg").exists() else ""
    )
    return JSONResponse(
        {
            "id": a.id,
            ...
            "hierarchy_rank": a.hierarchy_rank or 30,
...
async def generate_team(request: Request):
    ...
    if not prompt:
        return HTMLResponse(
            '<div class="msg-system-text">Prompt requis</div>', status_code=400
        )
...
async def agent_world_page(request: Request):
    ...
    all_active = []
    project_ids_seen: dict = {}  # project_id → display name
    for mr in all_runs:
        st = _s(mr.status)
    ...
    # Filter by project if selected, else cap at 10 for performance
    if selected_project:
        active_missions = [m for m in all_active if m["project_id"] == selected_project]
    else:
        active_missions = all_active[:10]

    active_session_ids = {m["session_id"] for m in active_missions if m["session_id"]}

    # Recent messages from active sessions
    recent_messages = []
    ...
    projects_list = [{"id": k, "name": v} for k, v in sorted(project_ids_seen.items())]
    return _templates(request).TemplateResponse(
        "agent_world.html",
        {
            "request": request,
