...
async def _probe_node(url: str, client: httpx.AsyncClient) -> WorkerNodeInfo:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            info.load_score = float(
                d.get("load_score", (info.cpu_percent + info.ram_percent) / 2)
            )
...
async def select_worker_node() -> str | None:
    ...
    logger.info(
        "Dispatch: selected worker %s (cpu=%.1f%% ram=%.1f%% missions=%d)",
        best.url,
        best.cpu_percent,
        best.ram_percent,
        best.active_missions,
    )
    return best.url
...
# This is called from execution.py after creating the EpicRun locally.
...
async def maybe_dispatch(
    ...
    if "error" in result:
        logger.warning(
            "Dispatch to %s failed: %s — falling back to local",
            worker_url,
            result["error"],
        )
...
async def api_missions_dispatch(request: Request):
    ...
    Validates API key (if configured), creates local EpicRun, and starts orchestration.
