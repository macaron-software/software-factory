...
# ── AgeMem Memory RL endpoints ─────────────────────────────────────────────────
# WHY: exposes the 3-stage progressive RL training from AgeMem (arXiv:2601.01885).
...
async def memory_rl_train(request: Request) -> JSONResponse:
    ...
    Body: {"stage": 1|2|3, "max_rows": int (optional, default 10000)}
    """
    try:
        ...
        stage, max_rows = 1, 10_000

    try:
        from ...agents.rl_policy import get_memory_rl_policy
        import asyncio

        result = await asyncio.get_event_loop().run_in_executor(
            None, lambda: get_memory_rl_policy().train_stage(stage, max_rows)
        )
        return JSONResponse(result)
    except Exception as e:
        log.exception("memory_rl_train error")
        return JSONResponse({"error": str(e)}, status_code=500)


@router.get("/api/rl/memory/stats")
async def memory_rl_stats() -> JSONResponse:
    """Return current status of the AgeMem memory RL policy."""
    try:
        from ...agents.rl_policy import get_memory_rl_policy

        return JSONResponse(get_memory_rl_policy().stats())
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500)
