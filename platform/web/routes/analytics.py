...
# =============================================================================
# Darwin Team Fitness API  (A/B testing, leaderboard, OKR, evolution)
# =============================================================================
...
async def teams_leaderboard(
    ...
    """Ranked team list for a (technology, phase_type) context."""
    from ...patterns.team_selector import TeamSelector
    data = TeamSelector.get_leaderboard(technology=technology, phase_type=phase_type, limit=limit)
    return {"teams": data, "technology": technology, "phase_type": phase_type}


@router.get("/api/teams/okr")
async def teams_okr(technology: str = "", phase_type: str = "") -> dict:
    """OKR list, optionally filtered."""
    from ...db.migrations import get_db
    db = get_db()
    try:
        q = "SELECT * FROM team_okr WHERE 1=1"
        params = []
        if technology:
            q += " AND technology = ?"
        ...
        q += " ORDER BY team_key, technology, phase_type"
...
async def teams_okr_update(okr_id: int, payload: dict) -> dict:
    """Update an OKR target or current value."""
    from ...db.migrations import get_db
    db = get_db()
    try:
        fields = []
        vals = []
        for col in ("okr_text", "kpi_name", "kpi_target", "kpi_current", "kpi_unit", "period"):
            if col in payload:
                fields.append(f"{col} = ?")
        ...
        vals.append(okr_id)
        db.execute(f"UPDATE team_okr SET {', '.join(fields)}, updated_at = CURRENT_TIMESTAMP WHERE id = ?", vals)
        db.commit()
        return {"ok": True}
    finally:
        db.close()
...
async def teams_evolution(
    ...
    """Fitness history for up to 2 teams — for line charts."""
    from ...db.migrations import get_db
    ...
    try:
        ...
        q = """
            SELECT agent_id, pattern_id, snapshot_date, fitness_score, runs
            FROM team_fitness_history
            WHERE technology = ? AND phase_type = ?
              AND snapshot_date >= ?
        """
        params: list = [technology, phase_type, since]
        if agent_id:
            q += " AND agent_id = ?"
        ...
        q += " ORDER BY agent_id, pattern_id, snapshot_date"
        rows = db.execute(q, params).fetchall()
        return {"history": [dict(r) for r in rows], "since": since}
    finally:
        db.close()


@router.get("/api/teams/selections")
async def teams_selections(limit: int = 20) -> dict:
    """Recent Thompson Sampling selections."""
    from ...db.migrations import get_db
    db = get_db()
    try:
        rows = db.execute("""
...
async def teams_retire(agent_id: str, pattern_id: str, technology: str = "generic", phase_type: str = "generic") -> dict:
    """Soft-retire a team (force)."""
    from ...db.migrations import get_db
    from datetime import timezone
    db = get_db()
    try:
        db.execute("""
            UPDATE team_fitness SET retired = 1, retired_at = ?, weight_multiplier = 0.1
            WHERE agent_id = ? AND pattern_id = ? AND technology = ? AND phase_type = ?
        """, (datetime.now(timezone.utc).isoformat(), agent_id, pattern_id, technology, phase_type))
        db.commit()
        return {"ok": True, "action": "retired", "agent_id": agent_id, "pattern_id": pattern_id}
    finally:
        db.close()


@router.post("/api/teams/{agent_id}/{pattern_id}/unretire")
async def teams_unretire(agent_id: str, pattern_id: str, technology: str = "generic", phase_type: str = "generic") -> dict:
    """Un-retire a team (restore to active)."""
    from ...db.migrations import get_db
    db = get_db()
    try:
        db.execute("""
            UPDATE team_fitness SET retired = 0, retired_at = NULL, weight_multiplier = 1.0
            WHERE agent_id = ? AND pattern_id = ? AND technology = ? AND phase_type = ?
        """, (agent_id, pattern_id, technology, phase_type))
        db.commit()
        return {"ok": True, "action": "unretired", "agent_id": agent_id, "pattern_id": pattern_id}
    finally:
        db.close()


@router.get("/api/teams/ab-tests")
async def teams_ab_tests(status: str = "", limit: int = 20) -> dict:
    """List A/B shadow tests."""
    from ...db.migrations import get_db
    db = get_db()
    try:
        q = "SELECT * FROM team_ab_tests"
        params = []
        if status:
            q += " WHERE status = ?"
        ...
        params.append(limit)
        rows = db.execute(q, params).fetchall()
        return {"ab_tests": [dict(r) for r in rows]}
    finally:
        db.close()


@router.get("/api/teams/contexts")
async def teams_contexts() -> dict:
    """List all (technology, phase_type) combos that have fitness data."""
    from ...db.migrations import get_db
    db = get_db()
    try:
        rows = db.execute("""
            SELECT DISTINCT technology, phase_type, COUNT(*) as teams, MAX(runs) as max_runs
            FROM team_fitness
            GROUP BY technology, phase_type
            ORDER BY technology, phase_type
        """).fetchall()
        return {"contexts": [dict(r) for r in rows]}
    finally:
        db.close()
