...
def _today_count() -> int:
    ...
    row = db.execute(
        "SELECT COUNT(*) FROM chaos_runs WHERE ts LIKE ?", (f"{today}%",)
    ).fetchone()
    db.close()
...
def get_chaos_history(limit: int = 50) -> list[dict]:
    db = get_connection()
    rows = db.execute(
        "SELECT * FROM chaos_runs ORDER BY ts DESC LIMIT ?", (limit,)
    ).fetchall()
    db.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--once", action="store_true", help="Run one random scenario and exit"
    )
    parser.add_argument("--scenario", type=str, help="Specific scenario to run")
    parser.add_argument(
        "--loop", action="store_true", help="Start continuous chaos loop"
    )
