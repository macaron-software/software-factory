...
from ..models import EpicRun, EpicStatus, PhaseRun
...
class EpicStore:
    ...
    def get_mission(self, mission_id: str) -> Optional[MissionDef]:
        ...
        try:
            row = db.execute(
                "SELECT * FROM epics WHERE id = ?", (mission_id,)
            ).fetchone()
    ...
    def create_mission(self, m: MissionDef) -> MissionDef:
        ...
        try:
            db.execute(
                """INSERT INTO epics (id, project_id, name, description, goal, status, type,
                   workflow_id, parent_epic_id, wsjf_score, created_by, config_json,
    ...
    def update_mission_status(self, mission_id: str, status: str) -> bool:
        ...
        try:
            ...
            if kanban:
                cur = db.execute(
                    "UPDATE epics SET status = ?, completed_at = ?, kanban_status = ? WHERE id = ?",
                    (status, completed, kanban, mission_id),
                )
            else:
                cur = db.execute(
                    "UPDATE epics SET status = ?, completed_at = ? WHERE id = ?",
    ...
    def _notify_mission_status(self, mission_id: str, status: str, db):
        ...
        try:
            row = db.execute(
                "SELECT name, project_id, type FROM epics WHERE id = ?",
                (mission_id,),
            ).fetchone()
            if not row:
                return
    ...
    def _update_darwin_fitness(self, mission_id: str, status: str, db):
        ...
        try:
            mission = db.execute(
                "SELECT id, type, config_json FROM epics WHERE id = ?", (mission_id,)
            ).fetchone()
            if not mission:
                return
    ...
    def list_children(self, parent_epic_id: str) -> list[MissionDef]:
        ...
        try:
            rows = db.execute(
                "SELECT * FROM epics WHERE parent_epic_id = ? ORDER BY wsjf_score DESC",
                (parent_epic_id,),
            ).fetchall()
            return [_row_to_mission(r) for r in rows]
        finally:
    ...
    def update_mission(self, m: MissionDef) -> bool:
        ...
        try:
            cur = db.execute(
                """UPDATE epics SET name=?, description=?, goal=?, status=?, type=?,
    ...
    def portfolio_stats(self) -> list[dict]:
        ...
        try:
            rows = db.execute("""
                ...
                FROM epics m GROUP BY m.project_id ORDER BY m.project_id
            """).fetchall()
            results = []
            for r in rows:
                pid = r["project_id"]
                task_rows = db.execute(
                    ...
                    WHERE mission_id IN (SELECT id FROM epics WHERE project_id = ?)
                    GROUP BY status
                """,
                    (pid,),
                ).fetchall()
...
_store: Optional[EpicStore] = None


def get_epic_store() -> EpicStore:
    global _store
    if _store is None:
        _store = EpicStore()
    return _store
...
class EpicRunStore:
    ...
    def create(self, run: EpicRun) -> EpicRun:
        db = get_db()
        try:
            db.execute(
    ...
    def get(self, run_id: str) -> Optional[EpicRun]:
        db = get_db()
        try:
            row = db.execute(
                "SELECT * FROM epic_runs WHERE id = ?", (run_id,)
            ).fetchone()
            return _row_to_epic_run(row) if row else None
        finally:
            db.close()
    ...
    def list_runs(self, project_id: str = "", limit: int = 20) -> list[EpicRun]:
        ...
        try:
            if project_id:
                rows = db.execute(
                    "SELECT * FROM epic_runs WHERE project_id = ? ORDER BY created_at DESC LIMIT ?",
                    (project_id, limit),
                ).fetchall()
            else:
                rows = db.execute(
    ...
    def update(self, run: EpicRun) -> bool:
    ...
    def update_phase(self, run_id: str, phase_id: str, **kwargs) -> Optional[EpicRun]:
    ...
    def delete(self, run_id: str) -> bool:
        db = get_db()
        try:
            cur = db.execute("DELETE FROM epic_runs WHERE id = ?", (run_id,))
            db.commit()
            return cur.rowcount > 0
        finally:
            db.close()

    def list_children_runs(self, parent_id: str) -> list[EpicRun]:
        """List sub-mission runs linked to a parent mission run."""
        db = get_db()
        try:
            rows = db.execute(
                "SELECT * FROM epic_runs WHERE parent_epic_id = ? ORDER BY created_at",
                (parent_id,),
            ).fetchall()
            return [_row_to_epic_run(r) for r in rows]
        except Exception:
...
_run_store: Optional[EpicRunStore] = None


def get_epic_run_store() -> EpicRunStore:
    global _run_store
    if _run_store is None:
        _run_store = EpicRunStore()
    return _run_store
