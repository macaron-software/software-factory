...
def on_deploy_completed(project_id: str, epic_mission_id: str):
    ...
    from .store import get_epic_store
    ...
    secu = next(
        (
            m
            for m in missions
            if m.type == "security" and (m.config or {}).get("auto_provisioned")
        ),
        None,
    )
...
def on_tma_incident_fixed(project_id: str, incident_key: str):
    ...
    from .store import MissionDef, get_epic_store
    ...
    debt = next(
        (
            m
            for m in missions
            if m.type == "debt" and (m.config or {}).get("auto_provisioned")
        ),
        None,
    )
    if not debt:
...
def create_platform_incident(
    ...
    try:
        ...
        db.commit()
        logger.info(
            "Created platform_incident %s: %s (%s)", inc_id, title[:80], severity
        )
...
def on_phase_failed(
    ...
    severity = (
        "P2"
        if any(k in phase_name.lower() for k in ("deploy", "prod", "security"))
        else "P3"
    )
...
def on_deploy_failed(project_id: str, epic_mission_id: str, error: str = ""):
    ...
    incident = MissionDef(
        ...
        project_id=project_id,
        workflow_id="sf-pipeline",
        parent_epic_id=tma.id if tma else None,
        wsjf_score=18,
        created_by="deploy_pipeline",
        config={
            "auto_created": True,
            "deploy_error": True,
            "source_mission": epic_mission_id,
        },
    )
    incident = ms.create_mission(incident)
    logger.info(
        "Created deploy failure incident %s for project %s", incident.id, project_id
    )
    emit_reaction(
...
def on_security_alert(
    ...
    existing = [
        m
        for m in missions
        if m.type == "bug" and (m.config or {}).get("cve_id") == cve_id
    ]
    if existing:
        return existing[0]
    ...
    logger.info(
        "Created security bug %s for %s (%s) in project %s",
        bug.id,
        cve_id,
        severity,
        project_id,
