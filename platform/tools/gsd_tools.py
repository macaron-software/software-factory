...
_NEXT_KW = re.compile(
    r"\b(next|TODO|should|need to|will|plan)\b", re.IGNORECASE
)
    ...
    async def execute(self, params: dict, agent: "AgentInstance" = None) -> str:
        ...
        if isinstance(messages, str):
            try:
                ...
                return json.dumps({"error": "messages must be a JSON list of {role, content}"})
        ...
        summary_chars = (
            len(summary)
            + sum(len(s) for s in decisions + blockers + progress + next_steps)
        )
        reduction_pct = (
...
class GsdApplyPatternsTool(BaseTool):
    ...
    async def execute(self, params: dict, agent: "AgentInstance" = None) -> str:
        ...
        try:
            filled = template.format(task_description=task_description or "(no description provided)")
