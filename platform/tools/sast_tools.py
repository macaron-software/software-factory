...
_REGEX_PATTERNS = [
    (r"subprocess\.[a-z_]+\([^)]*shell\s*=\s*True", "S603 subprocess with shell=True — use list args to prevent injection"),
    (r"open\s*\([^)]+\)(?!\s*(?:as|,))", "M001 open() without 'with' — resource leak risk"),
    (r"requests\.(get|post|put|delete|patch)\s*\([^)]*\)(?!.*timeout)", "S113 HTTP request without timeout — can hang forever"),
    (r"random\.(random|randint|choice|shuffle)\s*\(", "S311 Standard PRNG not cryptographically secure — use secrets module for security tokens"),
    (r"hashlib\.(md5|sha1)\s*\(", "S303 MD5/SHA1 are cryptographically weak — use SHA256+"),
    (r"DEBUG\s*=\s*True", "S201 DEBUG=True in production code"),
    (r"""(password|secret|api_key|token)\s*=\s*['"][^'"]{4,}['"]""", "S105 Hardcoded credential detected"),
    (r"CORS.*origins.*\*|allow_origins.*\[.*\*", "S001 CORS wildcard (*) allows any origin — restrict to known domains"),
]
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.Call):
            ...
            if call_name in _DANGEROUS_CALLS:
                issues.append(
                    f"{_DANGEROUS_CALLS[call_name]} "
                    f"({filename}:{node.lineno})"
                )

        # Global mutable state: module-level list/dict assignments
        if isinstance(node, ast.Assign) and isinstance(node.col_offset, int) and node.col_offset == 0:
...
class SastCheckTool(BaseTool):
    ...
    async def execute(self, params: dict, agent: AgentInstance = None) -> str:
        ...
        # Semgrep (async, optional)
        semgrep_issues = await _run_semgrep(path)
        ...
        if classified["warnings"]:
            lines.append(f"WARNINGS ({len(classified['warnings'])} issues — should fix):")
