...
class SnykSearchTool(BaseTool):
    ...
    async def execute(self, params: dict, agent: "AgentInstance" = None) -> str:
        ...
        try:
            ...
            return json.dumps({"package": package, "ecosystem": osv_ecosystem, "vulns": result})
...
class SnykVulnGetTool(BaseTool):
    ...
    async def execute(self, params: dict, agent: "AgentInstance" = None) -> str:
        ...
        try:
            data = _get_json(f"https://api.osv.dev/v1/vulns/{urllib.parse.quote(vuln_id)}")
            return json.dumps(
                {
                    "id": data.get("id", ""),
    ...
    async def execute(self, params: dict, agent: "AgentInstance" = None) -> str:
        ...
        try:
            url = (
                ...
                + urllib.parse.urlencode({"search": query, "limit": str(limit)})
            )
            data = _get_json(url)
            if not isinstance(data, list):
