...
def normalize_technology(fingerprint: str) -> str:
    """Extract a normalized technology key from a stack fingerprint string.

    Used to route Darwin/Thompson selection to the right tech specialist.
    Examples:
      "Node.js/TypeScript | angular=16, material=15" → "angular_16"
      "Rust (cargo) | axum=0.7"                      → "rust_axum"
      "Python | fastapi=0.110"                        → "python_fastapi"
      "Node.js | react=18"                            → "react_18"
      ""                                              → "generic"
    """
    import re
    ...
    # Angular (most specific first)
    m = re.search(r"angular[=@\s]+(\d+)", fp)
    if m:
        return f"angular_{m.group(1)}"
    if "angular" in fp:
        return "angular"

    # React / Next.js
    if "next" in fp and ("react" in fp or "next=" in fp or "nextjs" in fp):
        m = re.search(r"next[=@\s]+(\d+)", fp)
        return f"nextjs_{m.group(1)}" if m else "nextjs"
    m = re.search(r"react[=@\s]+(\d+)", fp)
    if m:
        return f"react_{m.group(1)}"
    if "react" in fp:
        return "react"

    # Vue
    m = re.search(r"vue[=@\s]+(\d+)", fp)
    if m:
        return f"vue_{m.group(1)}"
    if "vue" in fp:
        return "vue"
    ...
    # Rust (framework-specific)
    if "rust" in fp or "cargo" in fp:
        if "axum" in fp:
            return "rust_axum"
        if "actix" in fp:
            return "rust_actix"
    ...
    if "python" in fp or "django" in fp or "fastapi" in fp or "flask" in fp:
        if "django" in fp:
    ...
    if "node" in fp or "typescript" in fp:
        return "nodejs"

    # Go / Elixir / Ruby
    if "go " in fp or "golang" in fp:
        return "golang"
    if "elixir" in fp or "phoenix" in fp:
        return "elixir"
    if "ruby" in fp or "rails" in fp:
        return "ruby"
