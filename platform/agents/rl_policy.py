...
# ── AgeMem 3-stage progressive RL for memory management ──────────────────────
#
# WHY: AgeMem (arXiv:2601.01885) proposes training memory management in 3 stages:
#   Stage 1 — LTM quality: reward = quality improvement after memory_store ops
#   Stage 2 — STM efficiency: reward = context token reduction from prune/retrieve
#   Stage 3 — Joint: combined reward across both memory types
#
# Implementation: tabular reward tracking from phase_outcomes mem_* columns.
# Each stage trains a separate Q-table keyed on (pattern_id, mem_op_profile).
# The joint stage learns to trade off LTM quality vs STM token cost.
# Ref: AgeMem arXiv:2601.01885, 2026-03.

_MEM_STAGE_ACTIONS = ["store_more", "store_less", "prune_aggressive", "prune_conservative", "keep"]


class MemoryRLPolicy:
    """3-stage progressive RL policy for memory management (AgeMem).

    Stage 1 — LTM quality: trains on mem_store_count vs quality_score delta.
    Stage 2 — STM efficiency: trains on mem_prune_count vs rejection_count delta.
    Stage 3 — Joint: combined reward balancing quality + efficiency.

    Q-table key: (pattern_id, stage, mem_op_profile)
    mem_op_profile: discretised (store_rate, prune_rate) bucket
    """

    def __init__(self) -> None:
        self._q: dict[str, dict[str, float]] = {}
        self._stage: int = 1  # current active training stage (1, 2, or 3)
        self._loaded = False
    ...
    def train_stage(self, stage: int, max_rows: int = 10_000) -> dict[str, Any]:
        """Train the policy on phase_outcomes for the given stage (1, 2 or 3).

        Stage 1 — LTM: reward = quality_score if mem_store_count > 0 else -0.1
        Stage 2 — STM: reward = 1 - rejection_count/5 if mem_prune_count > 0 else -0.1
        Stage 3 — Joint: reward = 0.5 * quality_score + 0.5 * (1 - rejection_count/5)
        """
        ...
        try:
            db = get_db()
            rows = db.execute(
                """SELECT pattern_id, quality_score, rejection_count,
                          mem_store_count, mem_prune_count, mem_retrieve_count, success
                   FROM phase_outcomes
                   WHERE mem_store_count IS NOT NULL
                   ORDER BY id DESC LIMIT ?""",
                (max_rows,),
            ).fetchall()
            db.close()
        except Exception as e:
            log.warning("MemoryRL train stage %d: DB error %s", stage, e)
            return {"error": str(e)}

        updates = 0
        for row in rows:
            pattern_id = row["pattern_id"] or "unknown"
            quality = float(row["quality_score"] or 0)
            rejections = float(row["rejection_count"] or 0)
            store_c = int(row["mem_store_count"] or 0)
            prune_c = int(row["mem_prune_count"] or 0)

            # Discretise mem op profile into buckets
            store_rate = "high" if store_c > 3 else ("med" if store_c > 0 else "none")
            prune_rate = "high" if prune_c > 2 else ("med" if prune_c > 0 else "none")
            state_key = f"{pattern_id}|s{stage}|{store_rate}|{prune_rate}"

            # Compute stage-specific reward
            if stage == 1:
                reward = quality if store_c > 0 else -0.1
                action = "store_more" if store_c == 0 else ("store_less" if quality < 0.5 else "keep")
            elif stage == 2:
                eff = max(0.0, 1.0 - rejections / 5.0)
                reward = eff if prune_c > 0 else -0.1
                action = "prune_aggressive" if rejections > 2 else ("prune_conservative" if rejections > 0 else "keep")
            else:  # stage 3 joint
                eff = max(0.0, 1.0 - rejections / 5.0)
                reward = 0.5 * quality + 0.5 * eff
                # Joint: balance store and prune
                if store_c == 0 and prune_c == 0:
                    action = "store_more"
                elif quality < 0.5:
                    action = "store_more"
                elif rejections > 2:
            ...
            if state_key not in self._q:
                self._q[state_key] = {a: 0.0 for a in _MEM_STAGE_ACTIONS}

            # Simple Q-update (learning rate 0.1, discount 0.9)
            current_q = self._q[state_key].get(action, 0.0)
            self._q[state_key][action] = current_q + 0.1 * (reward - current_q)
            updates += 1

        self._stage = max(self._stage, stage)
        self._loaded = True
        log.info("MemoryRL stage %d trained: %d transitions, %d states", stage, updates, len(self._q))
        return {
            "stage": stage,
            "transitions": updates,
            "states": len(self._q),
            "actions": _MEM_STAGE_ACTIONS,
        }

    def recommend_memory_ops(self, pattern_id: str, recent_quality: float, recent_rejections: float) -> dict[str, Any]:
        """Recommend memory operation strategy for a pattern run."""
        if not self._loaded:
            return {"action": "keep", "fired": False, "stage": self._stage}

        store_rate = "none"  # conservative default for new runs
        prune_rate = "none"
        state_key = f"{pattern_id}|s{self._stage}|{store_rate}|{prune_rate}"

        q_row = self._q.get(state_key, {})
        if not q_row:
            return {"action": "keep", "fired": False, "stage": self._stage}

        best_action = max(q_row, key=lambda a: q_row[a])
        best_q = q_row[best_action]
        fired = abs(best_q) > 0.05  # only fire if q-value is meaningful
...
# ── Memory RL Singleton ───────────────────────────────────────────────────────
_mem_policy: MemoryRLPolicy | None = None


def get_memory_rl_policy() -> MemoryRLPolicy:
    global _mem_policy
    if _mem_policy is None:
        _mem_policy = MemoryRLPolicy()
    return _mem_policy
