...
Seven enforcement layers:
  0. Scope check: platform agents cannot write code; project agents cannot call platform tools
  1. Tool ACL: agent can only call tools in its allowed_tools list
  2. Write permission: can_write flag in agent.permissions
  3. Execute permission: can_execute flag in agent.permissions
  4. Rate limits: max tool calls and writes per agent per session
  5. Path sandbox: file tools restricted to project workspace + allowed_paths
  6. Git path guard: git tools restricted to project_path
  7. Audit log: every denial is logged to SQLite for debugging

Scope values (agent.permissions["scope"]):
  platform — Jarvis/CTO: read-only observer, cannot write code or run builds
  art      — ART lead/RTE: project-scoped across ART
  project  — default: cannot call platform orchestration tools (create_project etc.)
  self     — AC/evolution: PLATFORM_ROOT only, sensitive subdirs (auth/rbac/data) blocked

Usage in tool_runner._run_tool():
    denied = get_permission_guard().check(agent_id, tool_name, args, ..., project_id=ctx.project_id)
    if denied:
...
# ── Scope enforcement ──
# Platform-only tools: only scope=platform agents may use these
_PLATFORM_ONLY_TOOLS = frozenset(
    {
        "platform_agents",
        "platform_missions",
        "create_project",
...
# Sensitive platform subdirs that scope=self agents cannot write to
_SELF_PROTECTED_SUBDIRS = frozenset(
    {
        "auth",
        "rbac",
        "security",
...
class PermissionGuard:
    ...
    def check_scope(
        ...
        """Layer 0: scope-based enforcement before ACL checks.

        scope=platform  Jarvis/CTO: read-only, cannot write code or run builds
        scope=project   default: cannot call platform orchestration tools
        scope=art       same as project but cross-ART read allowed
        scope=self      AC/evolution: PLATFORM_ROOT only, sensitive subdirs blocked
        """
        scope = (permissions or {}).get("scope", "project")

        # scope=platform: delegates writes to sub-agents — cannot call dev tools directly
        if scope == "platform" and tool_name in (_FILE_WRITE_TOOLS | _EXEC_TOOLS):
            reason = f"scope=platform cannot execute {tool_name} directly (delegate to project agents)"
            denial = PermissionDenial(
                agent_id=agent_id, tool_name=tool_name, reason=reason
            )
            self._log_denial(denial, session_id, str(args)[:200])
            return f"Permission denied: {reason}"

        # scope=project|art: cannot call platform orchestration tools
        if scope in ("project", "art") and tool_name in _PLATFORM_ONLY_TOOLS:
            reason = f"scope={scope} cannot call {tool_name}: contact Jarvis/CTO to orchestrate"
            denial = PermissionDenial(
                agent_id=agent_id, tool_name=tool_name, reason=reason
            )
            self._log_denial(denial, session_id, str(args)[:200])
        ...
        # scope=self: AC/evolution — protect sensitive platform subdirs from writes
        if scope == "self" and tool_name in _FILE_WRITE_TOOLS:
            path = args.get("path", "") or args.get("file_path", "")
            path_parts = str(path).replace("\\", "/").split("/")
            for part in path_parts:
                if part in _SELF_PROTECTED_SUBDIRS:
                    reason = f"scope=self cannot write to protected subdir: {part}/"
                    denial = PermissionDenial(
                        agent_id=agent_id,
                        tool_name=tool_name,
                        reason=reason,
    ...
    def check(
        ...
        # 0. Scope enforcement (platform/project/art/self)
        denied = self.check_scope(
            agent_id, tool_name, args, permissions, project_id, session_id
        )
        if denied:
            return denied
