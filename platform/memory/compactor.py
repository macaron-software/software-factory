...
def run_compaction() -> CompactionStats:
    ...
    def _stale_clause(col: str, days: int) -> str:
        if _pg:
            return f"NOW() - {col}::timestamp > INTERVAL '{days} days'"
...
def get_memory_health() -> dict:
    ...
    try:
        ...
        try:
            project_avg_rel = (
                conn.execute(
                    "SELECT CAST(round(avg(COALESCE(relevance_score,confidence))::numeric,3) AS FLOAT) as v FROM memory_project"
                ).fetchone()
                or {}
            ).get("v", 0)
            project_low_rel = (
        ...
        project_by_cat = [
            dict(r)
            for r in conn.execute(
                "SELECT category, count(*) as n, CAST(round(avg(confidence)::numeric,2) AS FLOAT) as avg_conf "
        ...
        global_dupes = (
            q(
                "SELECT count(*) as n FROM (SELECT key FROM memory_global GROUP BY key HAVING count(*)>1) AS _dupes"
            )
            or {}
        ).get("n", 0)
        global_by_cat = [
            ...
            for r in conn.execute(
                "SELECT category, count(*) as n, CAST(round(avg(confidence)::numeric,2) AS FLOAT) as avg_conf, "
                "CAST(round(avg(occurrences)::numeric,1) AS FLOAT) as avg_occ "
