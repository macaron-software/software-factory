...
class MemoryManager:
    ...
    def project_store(
        ...
        existing = conn.execute(
            "SELECT id FROM memory_project WHERE project_id=? AND category=? AND key=? AND COALESCE(agent_role,'')=?",
        ...
        if existing:
            ...
            cur = conn.execute(
                "INSERT INTO memory_project (project_id, category, key, value, confidence, source, agent_role) VALUES (?,?,?,?,?,?,?)",
                (project_id, category, key, value, confidence, source, agent_role),
            )
            conn.commit()
            rid = cur.lastrowid
        conn.close()
    ...
    def global_get(self, category: Optional[str] = None, limit: int = 50) -> list[dict]:
        ...
        if category:
            ...
            rows = conn.execute(
                "SELECT * FROM memory_global ORDER BY updated_at DESC LIMIT ?", (limit,)
            ).fetchall()
        conn.close()
        return [dict(r) for r in rows]
