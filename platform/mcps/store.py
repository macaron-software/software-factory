...
class MCPStore:
    ...
    def update_status(self, mcp_id: str, status: str):
        with self._conn() as conn:
            conn.execute(
                "UPDATE mcps SET status=?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
                (status, mcp_id),
            )
            conn.commit()
    ...
    def update_tools(self, mcp_id: str, tools: list[dict]):
        with self._conn() as conn:
            conn.execute(
                "UPDATE mcps SET tools_json=?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
...
BUILTIN_MCPS = [
        ...
        tools=[
            {
                ...
                "description": "Create named entities with type and observations",
    ...
    MCPServer(
        ...
        tools=[
            ...
            },
            {
                ...
                "description": "Take PNG screenshot of current page",
    ...
    MCPServer(
        ...
        tools=[
            {
                "name": "github_issues",
                "description": "List/search issues",
                "params": {
                    "owner": "string",
                    "repo": "string",
                    "state": "open|closed",
    ...
    MCPServer(
        ...
        description="Design System Solaris — Figma components, variants, WCAG patterns, design tokens, validation reports.",
        command="python3",
        args=["-m", "macaron_platform.mcp_solaris"],
        tools=[
            {
                "name": "solaris_component",
                "description": "Get Figma component details: all variants, properties, component sets",
                "params": {
            ...
            },
            {
                ...
                "params": {
                    "component": "string (required)",
                    "properties": "object (optional filter)",
                },
            },
            {
                "name": "solaris_wcag",
                ...
                "params": {
                    "pattern": "string (accordion|button|tabs|checkbox|combobox|dialog|radio-group|switch|breadcrumb|focus-visible|link|listbox|loader)"
                },
            },
            {
                "name": "solaris_knowledge",
                "description": "Query knowledge base: semantic HTML, WCAG patterns, DS best practices",
                "params": {
                    "category": "string (1-semantic-html|2-wcag-patterns|3-ds-best-practices|4-interactive-behaviors)",
                    "topic": "string (optional)",
                },
            },
            {
                "name": "solaris_validation",
                "description": "Get validation status for a component from latest report",
                "params": {"component": "string (optional)"},
            },
            {
                "name": "solaris_grep",
                "description": "Search in generated CSS/HTML/SCSS files",
                "params": {
                    "pattern": "string (regex)",
                    "file_type": "string (css|html|scss|all)",
                },
            },
            {
                "name": "solaris_list_components",
                "description": "List all 41 Figma components/families",
                "params": {},
            },
            {
                "name": "solaris_stats",
    ...
    MCPServer(
        id="mcp-jira",
        name="Jira",
        description="Accès anonymisé au Jira (RGPD Art. 25). Recherche JQL, backlog, sprints, épics.",
        command="python3",
    ...
    MCPServer(
        id="mcp-confluence",
        name="Confluence",
        description="Accès anonymisé au Confluence (RGPD Art. 25). Pages, espaces, recherche full-text.",
        command="python3",
