...
GITLAB_URL = os.getenv("GITLAB_ENTERPRISE_URL", "")
GITLAB_TOKEN = os.getenv("GITLAB_ENTERPRISE_TOKEN", "")
GITLAB_PROJECT = os.getenv("GITLAB_ENTERPRISE_PROJECT", "org/software-factory")
...
def upsert_page(slug: str, title: str, content: str) -> dict:
    ...
    r = httpx.put(
        _api(f"/{slug}"), headers=_headers(), json=payload, timeout=15, verify=False
    )
    if r.status_code == 404:
        r = httpx.post(
            _api(""), headers=_headers(), json=payload, timeout=15, verify=False
        )
    r.raise_for_status()
    return r.json()
...
def build_home_page(pages: list[dict]) -> str:
    ...
    lines = [
        ...
        "",
        "> Documentation de la **Plateforme Agents IA** — architecture multi-agents pour l'ingénierie logicielle.",
        "",
        "## Table des matières",
        "",
    ]
    for cat, items in sorted(by_cat.items(), key=lambda x: x[0]):
        ...
        cat_clean = (
            cat.split(" ", 1)[-1] if cat[0].isdigit() or not cat[0].isalpha() else cat
        )
...
if __name__ == "__main__":
    ...
    if not GITLAB_TOKEN:
        print("ERROR: GITLAB_ENTERPRISE_TOKEN not set")
        sys.exit(1)

    print(
        f"Syncing SF wiki → {GITLAB_URL}/{GITLAB_PROJECT} wiki {'(DRY RUN)' if dry else ''}"
    )
