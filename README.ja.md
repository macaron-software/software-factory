...
192のAIエージェントが構造化されたワークフローを通じて協働する**仮想ソフトウェアファクトリー**と考えてください。SAFe方法論、TDDプラクティス、自動品質ゲートに従います。

### 主な特徴

- **192の専門エージェント** — アーキテクト、開発者、テスター、SRE、セキュリティアナリスト、プロダクトオーナー
...
- **LLM回復力** — マルチプロバイダーフォールバック、ジッター付きリトライ、レート制限認識；推論/アーキテクチャにgpt-5.2、コード/TDDにgpt-5.2-codex、ドキュメント/ディスカッションにgpt-5-mini
...
- **マルチノードクラスター** — マスター/スレーブトポロジー、共有PostgreSQL、パッシブフェイルオーバー、トップバーのリアルタイムノードバッジ
...
<tr>
<td width="50%">
<strong>設定ハブ — 並行実行数、予算 & YOLO モード (/settings)</strong><br>
<img src="docs/screenshots/en/settings.png" alt="設定" width="100%">
</td>
<td width="50%">
<strong>ミッション司令センター — リアルタイムコックピット (/cockpit)</strong><br>
<img src="docs/screenshots/en/cockpit.png" alt="ミッションコックピット" width="100%">
</td>
</tr>
<tr>
<td width="50%">
<strong>ミッションボード — 全ミッション & 実行一覧 (/missions)</strong><br>
<img src="docs/screenshots/en/missions.png" alt="ミッションボード" width="100%">
</td>
<td width="50%">
<strong>メモリ & ナレッジ — 4層 RAG (/memory)</strong><br>
<img src="docs/screenshots/en/memory.png" alt="メモリ" width="100%">
</td>
</tr>
<tr>
<td width="50%">
<strong>ワークフロー — 46 組み込みパイプライン (/workflows)</strong><br>
<img src="docs/screenshots/en/workflows.png" alt="ワークフロー" width="100%">
</td>
<td width="50%">
<strong>OPS — 自動修復 & インフラ (/ops)</strong><br>
<img src="docs/screenshots/en/ops.png" alt="OPS ダッシュボード" width="100%">
</td>
</tr>
<tr>
<td width="50%">
<strong>RBAC — ロールベースアクセス制御 (/rbac)</strong><br>
<img src="docs/screenshots/en/rbac.png" alt="RBAC" width="100%">
</td>
<td width="50%">
<strong>プロダクトバックログ — Epics、Features & Stories (/backlog)</strong><br>
<img src="docs/screenshots/en/backlog.png" alt="バックログ" width="100%">
</td>
</tr>
</table>
...
**ネイティブSwiftUI macOSアプリ**（Apple SiliconおよびRosetta 2対応）をWebプラットフォームと併用できます。完全オフライン対応のローカルSFインスタンスとOVH/Azureへのリモート接続を提供します。
...
> macOSがブロックする場合：**システム設定 → プライバシーとセキュリティ → このまま開く**

### 機能
- **ローカルSF内蔵** — Ollama/MLX（Qwen、Llama等）またはAPIキーでエージェントを実行
- **3つのリモートコネクタ** — ローカルSF / OVHデモ / Azure本番（HA 2ノード）
- **Web機能と100%同等** — エージェント、ミッション、プロジェクト、TMA、Wiki、分析、DORA、Evals、アイデア出し、RBAC
- **22ユニットテスト** — WSJF、LLMConfig、オンボーディング、インスタンス管理
...
### エージェント・マーケットプレイス
- **192エージェントをカタログ化** — `/marketplace` でフルテキスト検索、ART/ロール/スキルでフィルタリング
...
`/settings` での集中プラットフォーム設定:

- **ミッション並行実行数** — 実行セマフォをライブ調整（1〜10の並列ミッション）
- **予算上限** — ミッションランあたりのLLM最大支出を設定；超過したランは一時停止
- **自動再開ウォッチドッグ** — プラットフォーム再起動時に一時停止中のミッションを自動再開
- **YOLOモード** — 完全自律実行のためにヒューマン・イン・ザ・ループ検証を無効化
- **永続的** — すべての設定はDBに保存され、サーバー再起動後も維持

### ミッション・コックピット

`/cockpit` でのリアルタイム司令センター:

- **ライブパイプラインビュー** — アイデア出し → ミッション → 実行中 → テスト → デプロイ済み
- **ミッションごとのステータス** — エージェント、進行中のツール呼び出し、現在までのLLMコスト
- **セマフォゲージ** — 空きスロットと使用中スロットを一目で確認
- **クイックコントロール** — コックピットからミッションの一時停止・再開・キャンセルが可能

### ミッションごとのLLMコスト追跡

ランごとの詳細な支出可視化:

- **ランごとの`llm_cost_usd`** — すべての実行が正確なLLM支出をUSDで記録
- **予算アラート** — ミッションが上限に達すると警告または強制停止
- **プロバイダー比較** — minimax / azure / openai での同一タスクのコストを比較
- **累積ダッシュボード** — プロジェクト、スプリント、日付範囲ごとにコストを集計

### アノテーションスタジオ — あらゆるページでビジュアルフィードバック

すべてのSFページをコラボレーティブなレビューキャンバスに変える組み込みビジュアルアノテーション機能：

- **✏️ アノテートボタン** — クリックでアノテーションモードを有効化；各ページにフローティングツールバーが表示
- **クリックでアノテート** — 任意の要素をクリック：バグ 🐛、コメント 💬、機能 ✨、デザイン 🎨、またはテキスト修正 📝
- **SAFe トレーサビリティバー** — トップバーのペルソナアイコン → **プログラム → エピック → フィーチャー → ストーリー**
- **ワイヤーフレームモード** — グリッドアイコン → スケルトン/シマービューでUX検査
- **チケット生成** — 各アノテーションからフィックスミッションを直接起動可能
- **テーマ適応** — CSS変数を使用し、ライト/ダークテーマに自動適応
...
- **マスター/スレーブトポロジー** — IHM + SSEはマスター専用；APIコールはnginx `least_conn`で全ノードに負荷分散
- **共有PostgreSQL** — 100% PostgreSQL、SQLiteゼロ；全ノードが同じデータベースを共有；Advisory Lockで同時起動時のスキーマ競合を防止
- **パッシブフェイルオーバー** — nginxは3回連続失敗後にノードをダウン扱いにする；トラフィックは自動的に健全なノードに転送、10秒後に復旧
- **クラスターノードレジストリ** — `platform_nodes`テーブルが各ノードを追跡：ロール、モード、URL、CPU%、MEM%、バージョン、ハートビート経過時間
- **ライブトップバーバッジ** — 各ノードをカラードットバッジで表示；緑 = オンライン（< 60秒）、赤 = 古い；HTMXで30秒ごとにポーリング
- **クリック時のポップオーバー詳細** — ノードバッジをクリックすると完全な診断情報を表示：ロール/モード、URL、CPU、MEM、最終確認時刻、バージョン
- **エージェントツール `platform_cluster`** — Jarvisと全エージェントがクラスターの健全性と負荷分散を自然言語で照会可能

### セキュリティとサンドボックス

- **Landlock ファイルシステムサンドボックス** — Linux Landlock LSM（カーネル5.13+）を使用してエージェントのシェル実行をワークスペースディレクトリに限定；非Linuxホストへの影響ゼロ
- **セキュリティ設定タブ** — 設定 → セキュリティからサンドボックスのオン/オフ切り替え、Landloadカーネルサポート状態の確認
- **ペンテストエージェントツール** — nmapポートスキャン、subfinderサブドメイン列挙、whatwebテックフィンガープリンティング、schemathesis APIファジング、SQLインジェクション、認証バイパス、SSRF検出

### コード品質とLLM

- **LLMコード品質強化** — SAST（bandit/semgrep）+ 循環的複雑度分析をすべてのコードレビューフェーズ前にエージェントコンテキストに注入
- **DeerFlowコンテキスト要約** — 再帰的コンテキスト圧縮 + 自動メモリ抽出（arXiv:2503.09516）；長時間のエージェントセッションでのトークン消費を削減
- **マルチモデルルーティング強化** — 推論/アーキテクチャに`gpt-5.2`、コード/TDDに`gpt-5.2-codex`、ディスカッション/ドキュメントに`gpt-5-mini`；ロールベース、`AZURE_CODEX_MODEL`で設定可能

### プラットフォームとSAFe

- **SAFe用語** — 全UIページでミッションをエピックに改名；ポートフォリオ統計にSAFe用語を表示（Epics / Features / Stories / Tasks）
- **インフラエスカレーション** — スプリントが必要なビルドツールを見つけられない場合、`ft-infra-lead`が自動起動してリトライ前にインストール
- **エージェントプランツール** — エージェントがマイルストーンとサブタスクを含む構造化プランを作成し、フェーズ間の継続性のためにメモリに保存
