...
def _submit_and_wait(message: str, timeout_s: int = 120) -> str:
    ...
    task = _post("/a2a/tasks", {
        "input": {"parts": [{"kind": "text", "text": message}]}
    })
    ...
    while time.time() < deadline:
        ...
        if state in ("completed", "failed", "canceled"):
            ...
            text = next((p.get("text", "") for p in output_parts if p.get("kind") == "text"), "")
...
def _handle_tool_call(name: str, args: dict) -> str:
    if name == "jarvis_ask":
        ...
        output_parts = (result.get("output") or {}).get("parts") or []
        text = next((p.get("text", "") for p in output_parts if p.get("kind") == "text"), "")
        return f"**Task {task_id[:8]}…** — state: `{state}`\n\n{text or '(no output yet)'}"
        ...
        for t in reversed(tasks):
            ...
            text = next(
                (p.get("text", "")[:80] for p in (t.get("input") or {}).get("parts", []) if p.get("kind") == "text"),
                "?"
            )
