...
Piensa en ello como una **fabrica de software virtual** donde 192 agentes IA colaboran a traves de flujos de trabajo estructurados, siguiendo la metodologia SAFe, practicas TDD y puertas de calidad automatizadas.

### Puntos clave

- **192 agentes especializados** — arquitectos, desarrolladores, testers, SREs, analistas de seguridad, product owners
- **46 workflows integrados** — ceremonias SAFe, quality gates, mantenimiento nocturno, seguridad, gestión del conocimiento
...
- **Resiliencia LLM** — fallback multi-proveedor, retry con jitter, gestion de rate-limit; gpt-5.2 para razonamiento/arquitectura, gpt-5.2-codex para código/TDD, gpt-5-mini para documentación/discusión
...
- **Clúster multi-nodo** — topología maestro/esclavo, PostgreSQL compartido, failover pasivo, badges de nodos en vivo en la topbar
...
<tr>
<td width="50%">
<strong>Centro de Configuración — Concurrencia, Presupuesto y Modo YOLO (/settings)</strong><br>
<img src="docs/screenshots/en/settings.png" alt="Configuración" width="100%">
</td>
<td width="50%">
<strong>Centro de Mando de Misiones — Cockpit en Tiempo Real (/cockpit)</strong><br>
<img src="docs/screenshots/en/cockpit.png" alt="Cockpit de Misión" width="100%">
</td>
</tr>
<tr>
<td width="50%">
<strong>Panel de Misiones — Todas las Misiones y Ejecuciones (/missions)</strong><br>
<img src="docs/screenshots/en/missions.png" alt="Panel de Misiones" width="100%">
</td>
<td width="50%">
<strong>Memoria y Conocimiento — RAG de 4 Capas (/memory)</strong><br>
<img src="docs/screenshots/en/memory.png" alt="Memoria" width="100%">
</td>
</tr>
<tr>
<td width="50%">
<strong>Flujos de Trabajo — 46 Pipelines Integrados (/workflows)</strong><br>
<img src="docs/screenshots/en/workflows.png" alt="Flujos de Trabajo" width="100%">
</td>
<td width="50%">
<strong>OPS — Auto-Reparación e Infraestructura (/ops)</strong><br>
<img src="docs/screenshots/en/ops.png" alt="Dashboard OPS" width="100%">
</td>
</tr>
<tr>
<td width="50%">
<strong>RBAC — Control de Acceso Basado en Roles (/rbac)</strong><br>
<img src="docs/screenshots/en/rbac.png" alt="RBAC" width="100%">
</td>
<td width="50%">
<strong>Backlog de Producto — Épicas, Features e Historias (/backlog)</strong><br>
...
Una **aplicación macOS nativa SwiftUI** (Apple Silicon y Rosetta 2) está disponible junto a la plataforma web, con instancia SF local offline y conectores remotos a OVH y Azure.
...
> Si macOS bloquea el binario: **Ajustes del sistema → Privacidad y seguridad → Abrir de todas formas**

### Características
- **SF local integrada** — agentes con Ollama/MLX (Qwen, Llama…) o claves API
- **3 conectores remotos** — SF local / demo OVH / prod Azure (HA 2 nodos)
- **100% ISO web** — Agentes, Misiones, Proyectos, TMA, Wiki, Analíticas, DORA, Evals, Ideación, RBAC
- **22 pruebas unitarias** — WSJF, LLMConfig, onboarding, gestión de instancias
...
### Darwin — Seleccion Evolutiva de Equipos
- **Seleccion Thompson Sampling** — seleccion probabilistica equipo agente+patron via `Beta(wins+1, losses+1)` por `(agent_id, pattern_id, tecnologia, tipo_fase)`
- **Fitness granular** — puntuacion separada por contexto: un equipo experto en migracion Angular puede ser malo en nuevas funcionalidades Angular
- **Fallback similaridad** — arranque en frio gestionado por prefijo tech (`angular_19` → `angular_*` → `generic`)
- **Jubilacion suave** — equipos debiles reciben `weight_multiplier=0.1`, depriorizados pero recuperables
- **Sistema OKR / KPI** — objetivos e indicadores por dominio y tipo de fase; 8 semillas por defecto
- **Tests A/B en shadow** — ejecuciones paralelas automaticas cuando dos equipos tienen puntuaciones cercanas (delta < 10) o al 10% de probabilidad
- **Dashboard Teams** en `/teams` — clasificacion con badges champion/rising/declining/retired, edicion OKR inline, graficas evolucion, historial selecciones, resultados A/B
- **Non-breaking opt-in** — `agent_id: "skill:developer"` activa Darwin; IDs explicitos sin cambios
...
### Mercado de Agentes
- **192 agentes catalogados** — busqueda de texto completo, filtro por ART/rol/habilidades en `/marketplace`
...
Configuración centralizada de la plataforma en `/settings`:

- **Concurrencia de misiones** — ajusta el semáforo de ejecución (1–10 misiones paralelas) en vivo
- **Límite de presupuesto** — establece el gasto LLM máximo por run; los runs que superan el presupuesto se pausan
- **Watchdog de auto-reanudación** — reanuda automáticamente las misiones pausadas al reiniciar
- **Modo YOLO** — desactiva la validación human-in-the-loop para runs totalmente autónomos
- **Persistente** — todos los ajustes almacenados en BD, sobreviven a reinicios

### Cockpit de Misiones

Centro de mando en tiempo real en `/cockpit`:

- **Vista de pipeline en vivo** — ideación → misiones → en ejecución → pruebas → desplegado
- **Estado por misión** — agente, llamadas a herramientas en progreso, coste LLM acumulado
- **Indicador de semáforo** — visualiza los slots libres vs ocupados de un vistazo
- **Controles rápidos** — pausar, reanudar, cancelar cualquier misión desde el cockpit

### Seguimiento de Costes LLM por Misión

Visibilidad granular del gasto por run:

- **`llm_cost_usd` por run** — cada ejecución registra el gasto LLM exacto en USD
- **Alertas de presupuesto** — advertencia o parada forzada cuando una misión alcanza su límite
- **Comparación de proveedores** — compara el coste de la misma tarea en minimax / azure / openai
- **Paneles acumulativos** — costes agregados por proyecto, sprint o rango de fechas

### Estudio de Anotaciones — Feedback Visual en Cualquier Página

Una capa de anotación visual integrada que convierte cualquier página de SF en un lienzo de revisión colaborativo:

- **✏️ Botón Anotar** — clic para activar el modo anotación; barra flotante en cada página
- **Clic para anotar** — clic en cualquier elemento: bug 🐛, comentario 💬, feature ✨, diseño 🎨 o corrección de texto 📝
- **Barra de trazabilidad SAFe** — icono persona en la barra superior → **Programa → Epic → Feature → Historias**
- **Modo wireframe** — icono rejilla → vista esqueleto/shimmer para inspección UX
- **Generación de tickets** — cada anotación puede lanzar una misión de corrección
- **Adaptativo al tema** — barra y ventana emergente usan variables CSS, se adaptan a claro/oscuro
...
## Novedades en v3.1.0 (marzo 2026)

### Clúster multi-nodo

- **Topología maestro/esclavo** — IHM + SSE exclusivamente en el maestro; llamadas API balanceadas entre todos los nodos via nginx `least_conn`
- **PostgreSQL compartido** — 100% PostgreSQL, cero SQLite; todos los nodos comparten la misma base de datos; advisory lock evita condiciones de carrera de esquema al arrancar simultáneamente
- **Failover pasivo** — nginx marca un nodo como caído tras 3 fallos consecutivos; el tráfico se enruta automáticamente a nodos sanos, recuperación tras 10 s
- **Registro de nodos del clúster** — tabla `platform_nodes` rastrea cada nodo: rol, modo, URL, CPU%, MEM%, versión, antigüedad del heartbeat
- **Badges en vivo en la topbar** — cada nodo se muestra como un punto de color; verde = en línea (< 60 s), rojo = obsoleto; sondeo cada 30 s via HTMX
- **Detalle en popover al hacer clic** — haz clic en cualquier badge de nodo para ver el diagnóstico completo: rol/modo, URL, CPU, MEM, última vez visto, versión
- **Herramienta de agente `platform_cluster`** — Jarvis y todos los agentes pueden consultar la salud del clúster y la distribución de carga en lenguaje natural

### Seguridad y Sandbox

- **Sandbox de sistema de archivos Landlock** — ejecución de shell del agente confinada al directorio workspace usando Linux Landlock LSM (kernel 5.13+); cero impacto en hosts no Linux
- **Pestaña de configuración de seguridad** — activa/desactiva el sandbox, consulta el estado de soporte del kernel Landlock desde Ajustes → Seguridad
- **Herramientas de agente Pentest** — escaneo de puertos nmap, enumeración de subdominios subfinder, fingerprinting de tecnologías whatweb, fuzzing de API schemathesis, inyección SQL, bypass de autenticación, detección de SSRF

### Calidad de código y LLM

- **Endurecimiento de calidad de código LLM** — SAST (bandit/semgrep) + análisis de complejidad ciclomática inyectados en el contexto del agente antes de cada fase de revisión de código
- **Sumarización de contexto DeerFlow** — compresión recursiva de contexto + extracción automática de memoria (arXiv:2503.09516); reduce el consumo de tokens en sesiones de agente de larga duración
- **Enrutamiento multi-modelo mejorado** — `gpt-5.2` para razonamiento/arquitectura, `gpt-5.2-codex` para código/TDD, `gpt-5-mini` para discusión/documentación; basado en roles, configurable via `AZURE_CODEX_MODEL`

### Plataforma y SAFe

- **Terminología SAFe** — misiones renombradas a epics en todas las páginas de UI; estadísticas del portfolio muestran términos SAFe (Epics / Features / Stories / Tasks)
- **Escalada de infra** — cuando un sprint no puede encontrar las herramientas de build necesarias, se lanza automáticamente `ft-infra-lead` para instalarlas antes de reintentar
- **Herramientas de plan de agente** — los agentes crean planes estructurados con hitos y sub-tareas, almacenados en memoria para continuidad entre fases
